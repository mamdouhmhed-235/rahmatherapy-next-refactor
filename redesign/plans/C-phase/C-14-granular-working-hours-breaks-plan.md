...
> **C-23 coordination (2026-07-16):** C-23 Phase A ports the `redesign/start-state` engine refactor onto master, which restructures this file into `loadSettings` → `loadContextRest` → `loadDayRecords(dates[])` (batched, multi-date) → `computeDaySlots` (pure). **If C-23 has landed first**, the line references below are stale: the global override fetch lives inside `loadDayRecords` and now loads *many* dates at once, so the `.maybeSingle()` → array widening must preserve the per-date grouping (`Map<date, …>`) rather than assume a single row for a single date — re-read the file before editing. C-23 also adds a defaulted options bag (`ignoreBookingWindow`, `ignorePublicPause`) used only by the admin month route; Phase D's `getBookingDateBounds` refactor sits alongside it without interacting. **If C-14 lands first**, C-23's port is no longer byte-identical — surface to the user and re-plan Phase A as a merge rather than a checkout. Whichever ships second re-reads the other's changes.
