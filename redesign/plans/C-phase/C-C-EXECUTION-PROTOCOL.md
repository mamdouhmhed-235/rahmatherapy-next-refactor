...
**Session model: Sonnet 5** (the Owner sets it via `/model` before issuing the goal). The orchestrator and all agents inherit Sonnet unless routed below. Mechanism: the `model` parameter on Task/agent dispatches and on workflow `agent()` calls.

**Routing table:**
- **Implementer agents on `opus`** for exactly these plans: **C-06** (RPC rewrite + highest-blast-radius migration), **C-04a** (largest plan; cron/delayed-email infrastructure), **C-11** (system-wide theming; public-flip risk lives here), **C-02** (greenfield schema + 2 RPCs + horizon cron), **C-23** (live public availability engine), **C-14** (live engine + atomic co-deploy migration).
...
- **All phase verifiers and closeout adversarial reviewers: Sonnet, high effort** (they judge against plan text, not from taste — Sonnet is sufficient and keeps the check independent).
- **The 4 programme drift checkpoints (§2.6): `opus`** — programme-wide judgment, 4 total, worth the spend.
