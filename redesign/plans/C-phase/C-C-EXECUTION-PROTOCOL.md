...
**Capability-based routing (Owner amendment 2026-08-02 — supersedes the earlier fixed split):** the orchestrator judges EVERY dispatch by task complexity and routes by capability. `sonnet` for tasks it can implement effectively with no issues — routine UI work, mechanical sweeps, straightforward components/actions, test authoring, and ALL verification/review agents. `opus` for tasks that genuinely exceed that — intricate schema/RPC logic, recurrence/date/timezone math, concurrency, subtle state machines, cross-cutting refactors, and edits to live customer surfaces. The hard phases of C-02/C-23/C-14 remain presumptively `opus`. Every `opus` dispatch logs a one-line justification in the progress file. Never choose the model to save tokens on work that needs the stronger one; never spend `opus` on work `sonnet` handles well; when genuinely uncertain about correctness-critical work, prefer `opus`.
