...
| C-01 | Google review post-completion email | ⏳ (brief + plan ✅) | — | — | — | C-B | item 1 — scope locked (see C-B-DECISIONS §3 C-01). **Brief + plan written 2026-05-26** — `redesign/briefs/C-01-review-request-email-brief.md` + `redesign/plans/C-phase/C-01-review-request-email-plan.md`. **Scheduler-engine deviation:** plan switches from C-B-DECISIONS Q4's pg_cron (not installed) to Cloudflare Workers cron (existing project pattern at `worker-entrypoint.ts` + `wrangler.jsonc`). Cadence (15 min) preserved. See brief §1.4 + §9.2 + plan §1 for rationale. Implementation ⏳ pending. |
