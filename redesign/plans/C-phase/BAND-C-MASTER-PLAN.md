...
| C-B | per-item brief + plan writing | ✅ | 2026-05-26 | 2026-05-26 | (this commit) | C-B-DECISIONS | covers **17 plans** (C-04b dropped, C-PRIVACY not written, C-FIELDWORK added, C-13 + C-14 added post-handoff via user direction, **C-15 + C-16 + C-17 added 2026-07-16 via user direction — plan-refinement phase**). Recommended order: C-06 → C-04a → C-05 → C-01 → C-FIELDWORK → C-11 → C-08 → **C-15** → C-13 → C-02 → C-09 → C-03 → C-07 → **C-16** → C-10, with **C-14 + C-17** independent (C-14 Phase D customer-picker and C-17 GA can each ship first; break phases anytime). **Progress: 17/17 ✅ COMPLETE** — original 12 landed 2026-05-26; C-13 + C-14 added same day; C-15 + C-16 + C-17 added 2026-07-16 (alongside the C-08 business-notifications + C-04a S7 amendments). C-C implementation is now unblocked. |
...
| C-17 | Google Analytics (GA4) on customer pages | ⏳ (brief + plan ✅) | — | — | — | C-B | **NEW 2026-07-16 (plan-refinement phase)** from user direction — GA4 tag `G-WM8BCYG060` on the customer-facing surfaces only. Smallest plan of the band: env-gated `GoogleAnalytics` component (`next/script`, `afterInteractive`, renders nothing outside production — dev/Playwright never pollute analytics), mounted in `(public)/layout.tsx` + a thin new `booking/layout.tsx` for `/booking/manage`; **admin never tracked** (gate asserts absence from admin HTML). Phase B: one `booking_request_submitted` conversion event on the booking success screen (`PreparedStep`), fire-once, zero PII. No new packages, zero migrations; one Cloudflare env var (build-time-inlined — pipeline injection verified). **Consent deliberately out of scope (user decision 2026-07-16)** — Google Consent Mode + banner is the user's declared NEXT refinement; the init block carries a marked insertion point. **Brief + plan written 2026-07-16** — `redesign/briefs/C-17-google-analytics-brief.md` + `redesign/plans/C-phase/C-17-google-analytics-plan.md`. **Sequencing:** fully independent (like C-14); ships anytime; branch confirmed with user at impl (public layouts diverge ~9 lines from the frontend line). Implementation ⏳ pending. |
