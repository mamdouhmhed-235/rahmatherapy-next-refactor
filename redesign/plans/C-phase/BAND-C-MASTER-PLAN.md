...
| C-B | per-item brief + plan writing | ✅ | 2026-05-26 | 2026-05-26 | (this commit) | C-B-DECISIONS | covers **14 plans** (C-04b dropped, C-PRIVACY not written, C-FIELDWORK added, C-13 + C-14 added post-handoff via user direction). Recommended order: C-06 → C-04a → C-05 → C-01 → C-FIELDWORK → C-11 → C-08 → **C-13** → C-02 → C-09 → C-03 → C-07 → C-10, with **C-14** independent (Phase D customer-picker can ship first; breaks phases anytime). **Progress: 14/14 ✅ COMPLETE** — original 12 landed 2026-05-26; C-13 + C-14 added same day via user-direction amendments. C-C implementation is now unblocked. |
...
| C-08 | Email templates + automation expansion | ⏳ (brief + plan ✅) | — | — | — | C-B | item 8 — **REFRAMED to 4 NEW + 1 existing-verify.** Decisions doc Q7 said "5 new templates" but code inspection confirmed staff_assignment already exists (renderer + send-fn + SUBJECTS + templates-data + 4 prod sends). C-08 ships 4 new (booking_confirmed_client + staff_unassignment + claim + client_assigned_therapist) + verifies existing staff_assignment. Per-row Resend tooling. Uses existing `email_template_overrides`. **No CHECK constraint migration needed** — verified email_delivery_events.event_type is free-text. **Brief + plan written 2026-05-26** — `redesign/briefs/C-08-email-automation-expansion-brief.md` + `redesign/plans/C-phase/C-08-email-automation-expansion-plan.md`. Implementation ⏳ pending. |
