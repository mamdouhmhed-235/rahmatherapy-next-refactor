...
| C-FIELDWORK | Fieldwork ergonomics (capability-keyed) | ⏳ (brief + plan ✅) | — | — | — | C-B | NEW from Q10 — `C-FIELDWORK-EXPERIENCE-plan.md`. Capability-keyed via `can_take_bookings` + active assignment. Booking-detail dual-view + `PractitionerTodaySection` (consumed by C-11) + `tel:`/maps/mobile-sticky primitives. **Brief + plan written 2026-05-26** — `redesign/briefs/C-FIELDWORK-EXPERIENCE-brief.md` + `redesign/plans/C-phase/C-FIELDWORK-EXPERIENCE-plan.md`. **Discovery during plan-writing:** tel: + maps + getGreeting all already exist in codebase — C-FIELDWORK lifts as canonical patterns rather than inventing new primitives. Implementation ⏳ pending. |
