...
### ✅ C-20 COMPLETE — 2026-08-09. Step 9's second half landed at `ded190b`

The blocker recorded here — *"C-20 is NOT marked ✅, and deliberately so — one plan STEP is genuinely unimplemented"* — is **resolved**. The reasoning is preserved because it was right: Google Maps loads client-side code, C-18 built the registry precisely so `/cookies` discloses everything the site loads, and shipping the autocomplete without an entry would have left that page understating reality — the same defect class C-18 was written to fix and C-19's closeout caught twice, in both directions.

**Owner decision, in chat 2026-08-09: option A — functional-on-interaction.** Maps loads only when a visitor focuses the address field; it is disclosed rather than consent-gated.

**Implemented and independently verified** (`redesign/evidence/C-20/step9-registry-verify.md`, PASS, zero blocking):
- Registry entry added to `src/lib/consent/cookie-registry.ts` (**not** `src/lib/cookie-registry.ts` — the plan's path is stale, see §0.1).
- `CONSENT_BANNER_VERSION` bumped `2026-07-16.1` → `2026-08-09.1`. Every returning visitor sees the banner once more; Owner-accepted.
- The bump also caught a latent bug: `KNOWN_BANNER_VERSIONS` was `[CONSENT_BANNER_VERSION]` only, so bumping would have **silently dropped the previous version**, breaking the file's own in-flight-beacon policy. Now `["2026-07-16.1", CONSENT_BANNER_VERSION]`.

**⚠️ Deviation from the literal decision wording, made deliberately and accepted by the orchestrator.** The entry is filed `purpose: "essential"`, not `"functional"`. `PURPOSE_DESCRIPTIONS.functional` makes a blanket promise — *"Nothing in this group is stored, or read back, unless you switch it on"* — and per the file's own GATING OBLIGATIONS note every other functional entry is backed by a real code gate. Under decision A, Maps loads **regardless** of consent state, so filing it functional would have made that group's promise false for this entry: over-disclosure in one direction, a false statement in the other. `essential` — *"Needed for a function you specifically asked for"* — is the Owner's own justification for A, expressed in this file's vocabulary. The orchestrator verified the premise directly (`PURPOSE_DESCRIPTIONS` at `cookie-registry.ts:324-331`; `GATED_PURPOSES` filters `essential`, so no dead toggle is created). **The substance of the Owner's decision is intact; the label differs. Reversible if the Owner prefers otherwise.**

**No cookie name was invented.** Google publishes none for this exact combination (JS API loader + Places API (New) Autocomplete, no map ever rendered), and the codebase's own implementation sets no cookie or storage itself — so the entry says so plainly rather than borrowing a name from a third-party scanner. Under-disclosure and over-disclosure were both treated as defects.

⏸ **Remaining open items, none blocking:** two observations awaiting an Owner view — the `area == city` duplication on unitary-authority addresses, and the `autoComplete="street-address"` vs `off` choice (§1a.2) — plus the small manual phone check in §1d.1. The key-rotation decision is **ANSWERED** (§0.4 item 1 — do not rotate).
