...
> **Refinement 2026-07-26** — re-verified against `master` @ `ea97932`; amended per decisions D21/D22 and findings C21-F1/F3/F4/F7/F8/F9 (see plan + refinement changelog).
...
**Predecessors:**
- Discovery 2026-07-16: while setting HTTP-referrer restrictions on the Google Maps key, the live domain was confirmed as **`rahmatherapy.uk`** — which appears **nowhere in the codebase**. Instead the code carries two wrong domains: `rahmatherapy.co.uk` (12 refs) and `rahmatherapy.com` (8 refs). *(Re-census 2026-07-26, C21-F3: now 15 × `.co.uk` in 12 files, 8 × `.com` in 5 files — the +3 `.co.uk` are the area pages shipped in merge `ea97932`.)*
...
| `src/app/(public)/about/page.tsx:21` *(anchor re-verified 2026-07-26, C21-F7)* | `https://rahmatherapy.co.uk/about` | Structured data |
| `src/app/(public)/services/page.tsx:23` | `https://rahmatherapy.co.uk/services` | Structured data |
| `src/app/(public)/reviews/page.tsx:24,30` *(anchors re-verified 2026-07-26, C21-F7)* | `https://rahmatherapy.co.uk/` + `/reviews` | Structured data (breadcrumb) |
| `src/app/(public)/faqs-aftercare/page.tsx:25` | `https://rahmatherapy.co.uk/faqs-aftercare` | Structured data |
| `src/app/(public)/areas/page.tsx:19` *(added 2026-07-26 — C21-F1)* | `https://rahmatherapy.co.uk/areas/` | Canonical (area hub) |
| `src/app/(public)/areas/[slug]/page.tsx:29` *(added 2026-07-26 — C21-F1)* | `https://rahmatherapy.co.uk/areas/<slug>/` | Canonical (5 area spokes) |
| `src/components/area-pages/area-json-ld.ts:3` *(added 2026-07-26 — C21-F1)* | file-local `const SITE_URL = "https://rahmatherapy.co.uk"` (used L10,21,31,32) | Structured data (Service/Breadcrumb) — must import the single-source module, not literal-swap |
...
1. **Single source of truth (the actual fix):** add `SITE_URL` to the existing site-content module (`src/content/site/`), sourced from an env var with a hard-coded production default, and make every absolute URL derive from it — `metadataBase`, all JSON-LD `url`/`item` values, anywhere else an absolute site URL is composed. After this, the domain exists in exactly ONE place. *(Amended 2026-07-26 — D21/C21-F2: NOT env-sourced — `SITE_URL` is a pure constant. `NEXT_PUBLIC_SITE_URL` remains the separate, throw-by-design env contract for email/cron link generation and is localhost in dev; the production env value is aligned in a HARD-STOP step — plan Step 1a.)*
...
5. **Guard:** a unit test asserting no hard-coded `rahmatherapy.co.uk` / `rahmatherapy.com` string survives in `src/` (a grep-style assertion) so drift can't silently return. *(Extended 2026-07-26 — D22/C21-F9: also asserts the live-domain literal `https://rahmatherapy.uk` appears in exactly one file, `src/content/site/site-url.ts`, so a duplicated correct-domain constant — the `area-json-ld.ts:3` drift mode — fails too.)*
...
- **Acceptance:** `SITE_URL` is the only place the domain appears; every public page's rendered canonical + JSON-LD shows `https://rahmatherapy.uk/...`; view-source spot-check on all 6 public pages *(2026-07-26 — C21-F4/D22: now all 12 — 6 original + 6 area pages; enumeration in plan §3.2)*; **the site's published contact address reads `rahmatherapy@outlook.com` and its `mailto:` link opens to it**; no `rahmatherapy.co.uk` / `rahmatherapy.com` literal survives in `src/`; the anti-drift test passes; no visual change beyond the corrected email text; static gates pass.
