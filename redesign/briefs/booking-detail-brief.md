...
2. **Participants** (`AdminPanel`, H2) — `ParticipantBreakdown`: each participant as an `AdminEntityRow` showing label, client gender, required therapist gender, services. "Same-gender required" Restricted-family chip per participant when applicable — always visible, never colour-only.
