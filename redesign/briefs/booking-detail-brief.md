...
2. **Participants** (`AdminPanel`, H2) — `ParticipantBreakdown`: if `booking.group_booking = true`, the panel header carries a Restricted-family "Group · {N} people" chip alongside the H2. Each participant as an `AdminEntityRow` showing: label (Urbanist 500), client gender (Pending-family chip), "Same-gender required" Restricted-family chip (always visible, never colour-only), and services for that participant. The "Unassigned" Attention-family chip reflects the BOOKING-LEVEL `assignment_status` field, not a per-participant flag — it appears on the Assignment panel (item 3), not per-participant row. **Booking state on arrival from booking-new:** bookings created via booking-new arrive as `status: "pending"` + `assignment_status: "unassigned"` unless the coordinator used the step 4 inline assignment panel. Both states are handled correctly by the existing AssignmentManager. **Phase 1 note:** mixed-gender group bookings with two gender sections in booking-new still produce ONE booking record; ParticipantBreakdown renders all participants in a flat list regardless of gender. Phase 2 may introduce gender-segregated sub-groups when `group_session_id` links exist.
