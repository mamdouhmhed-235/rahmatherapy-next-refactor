...
| `public/images/empty/staff-empty.svg` | — (TBD) | SVG | `alt="No staff yet"` | net-new asset — placeholder needed for `/admin/staff` `EmptyState` (two-people-with-plus illustration, ~80–120px, single-color so it tints via `currentColor`). Reused across all three scope variants (admin / coordinator / therapist); each scope supplies its own heading + body from staff-brief §8 empty-state table. Phase 6 staff session uses `data-redesign-needs-photo` until asset lands. | add-asset |
| `public/images/admin/empty-states/assignments-quiet.svg` | — (TBD) | SVG | `alt="No assigned bookings yet"` | net-new asset — placeholder for `/admin/staff/<id>` Panel L2 (Assigned bookings) `EmptyState` (calendar-quiet illustration, ~80–120px, single-color so it tints via `currentColor`). Phase 6 staff-detail session uses `data-redesign-needs-photo` until asset lands. | add-asset |
| `public/images/admin/empty-states/emails-empty.svg` | — (TBD) | SVG | `alt="No email events logged yet"` | net-new asset — placeholder needed for `/admin/emails` Delivery-tab `EmptyState` (envelope-with-check illustration, ~80–120px, single-color so it tints via `currentColor`). Phase 6 emails session leaves the Lucide `Inbox` glyph in place until the asset lands. | add-asset |
| `public/images/admin/empty-states/reminders-empty.svg` | — (TBD) | SVG | `alt="No upcoming bookings need a reminder"` | net-new asset — placeholder needed for `/admin/emails` Reminders-tab `EmptyState` (calendar-with-zzz illustration, ~80–120px, single-color so it tints via `currentColor`). Phase 6 emails session leaves the Lucide `CalendarClock` glyph in place until the asset lands. | add-asset |
| `public/images/admin/empty-states/templates-empty.svg` | — (TBD) | SVG | `alt="Select a template to preview"` | net-new asset — placeholder needed for `/admin/emails?tab=templates` no-selection `EmptyState` in the right preview panel (envelope illustration, ~80–120px, single-color so it tints via `currentColor`). Phase 6 email-templates session leaves the Lucide `Mail` glyph in place until the asset lands. | add-asset |
