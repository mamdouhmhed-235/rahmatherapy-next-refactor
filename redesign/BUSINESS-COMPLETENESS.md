...
**Engineering pause Session 4 close (2026-05-19):** Status flipped PARTIAL → HANDLED. **Evidence:** `FINAL-AUDIT.md` row P0-A1 (baseline-resolution table) recorded the Phase 7 Gate 1 verification — *"74 `role='alert'` callsites across 42 admin files — shared `FieldError` primitive (`ManualBookingForm.tsx:367-381`) reused; spot-check live on `/admin/clients/new`, `/admin/settings`, `/admin/bookings/[id]`, `/admin/login`"*. Fresh grep 2026-05-19 (post-Sessions 2 + 3): **95 `role="alert"` occurrences across 59 files in `src/app/admin/`** (up from FINAL-AUDIT's snapshot — Session 2's `TemplateEditForm` save-error and `ManualSendSheet` additions account for the uptick; Session 3's cron handler returns JSON errors, not `role='alert'` regions, so contributes zero to this count). The shared `FieldError` primitive is the canonical implementation; per-form regions inherit the full `role="alert" aria-live="polite" aria-atomic="true"` triplet by reuse.
...
`BLOCKS-REDESIGN · Zone 1 · HANDLED` · **Evidence:** `A11Y-BASELINE.md` global finding A5. `required` HTML attribute is present on `LoginForm`, `SettingsForm`, `ManualBookingForm`, `ClientCreateForm` etc. without an "*" or "(required)" visible marker. **Coverage:** 00-shared-components primitive shipped (`aa76451`); per-form audit live for: login (visible `*` markers in Cancelled-family colour with `aria-hidden="true"` on LoginForm.tsx per `7e7e930`).
...
**Engineering pause Session 4 close (2026-05-19):** Status flipped PARTIAL → HANDLED. **Evidence:** `FINAL-AUDIT.md` row P1-A4 (baseline-resolution table) recorded the Phase 7 Gate 1 verification — *"Live `/admin/settings` returned `requiredMarker: 5/5`; `/admin/bookings/new` returned `4/4`; shared `FieldLabel` adds `<span aria-hidden='true' className='ml-0.5 text-[oklch(26%_0.14_25)]'>*</span>`"*. Canonical primitive: `src/components/ui/form.tsx:39` (`export function FieldLabel`). Adoption confirmed via grep 2026-05-19 — `FieldLabel` consumers include `EnquiryForm`, `NewStaffForm`, `ManualBookingForm` (multiple callsites including `:951, :971, :995`), and is the shared label primitive every form-bearing page reaches for. Inline required-`*` markers outside `FieldLabel` (raw `aria-hidden="true"…>*` strings): **7 callsites** total — all wrapping `aria-hidden="true"` per the primitive's contract (`ManualBookingForm.tsx`, `StaffBlockedDatesManager.tsx`, `StaffAvailabilityOverridesManager.tsx`); these are pre-`FieldLabel` patterns that pre-date the primitive and still satisfy the WCAG attribute contract.
...
`BLOCKS-REDESIGN · Zone 2 · HANDLED with caveat` · **Evidence:** `sendBookingReminderEmail` template exists in `src/lib/email/notifications.ts:520` but is fired only from `/admin/emails` manual "Send reminder" buttons. Grep for `pg_cron`, `scheduled-functions`, `edge-function`: zero matches.

**Engineering pause Session 3 close (2026-05-19):** Status flipped NOT-STARTED → HANDLED with caveat. Cron trigger configured in `wrangler.jsonc` (`triggers.crons: ["0 8 * * *"]`, 08:00 UTC daily) — **activates on next Cloudflare production deploy**. Cron handler at `src/app/api/cron/booking-reminders/route.ts` invoked by a custom Cloudflare Worker `scheduled()` export in `worker-entrypoint.ts` via the existing `WORKER_SELF_REFERENCE` service binding. Re-uses `sendBookingReminderEmail` from `src/lib/email/notifications.ts:520` (template render + Resend + `email_delivery_events` insert via `sendTrackedEmail`); handler additionally writes `audit_logs` row with `action_type='manual_booking_reminder_sent'` and `after_state.automated=true` (temporary measure; future audit-taxonomy extension will split into a dedicated `automated_booking_reminder_sent`). Original Supabase Edge Function + `pg_cron` plan was superseded mid-session — see the "## 2026-05-19 Architecture amendment" section at the top of `BUILD-automated-booking-reminders.md` for the reasoning. **Live-invoke coverage limited to Phase B4 only due to Session 3 Zone-2 incident** (one real-customer reminder fired early during the test invoke — see ENGINEERING-LOG.md "Process note — Session 3 Zone-2 discipline failure #2"). Phase C smoke tests (idempotency, cancellation guard, missing env-var bonus) verified via code inspection + DB cross-check; Phase D cleanup + wrangler dry-run completed. **Transcript:** `/redesign/backend-smoke-tests/automated-booking-reminders-2026-05-19.txt`.
...
`NICE-TO-HAVE → BLOCKS-REDESIGN (see 2A-16) · Zone 2 · HANDLED with caveat` · **Evidence:** `grep -l "pg_cron\|scheduled-functions\|edge-function"` returns zero matches across the repo.

**Engineering pause Session 3 close (2026-05-19):** Cross-listed with 2A-16 — same engineering effort, same evidence. The "scheduled function infrastructure" gap was filled by Cloudflare Cron Triggers (`wrangler.jsonc` `triggers.crons` block + `scheduled()` export in `worker-entrypoint.ts`), NOT by Supabase Edge Functions + `pg_cron`. This codebase deploys as a single Cloudflare Worker via `@opennextjs/cloudflare`, so the Cloudflare-native path was the architecturally simpler choice. Activates on next Cloudflare production deploy. Same caveat: live-invoke coverage limited to Phase B4 only; remaining smoke tests via code inspection. **Transcript:** `/redesign/backend-smoke-tests/automated-booking-reminders-2026-05-19.txt`.
