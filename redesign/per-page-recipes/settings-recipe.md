...
Use `run_in_background: true`. Poll `http://localhost:3024/admin/settings` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/settings-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during a save + an intake toggle — verify same endpoint as `/redesign/RECON.md` baseline (POST to `updateBusinessSettings` server action)
...
8. `DEV_SERVER_READY at http://localhost:3024`
...
13. `BG_WHITE_HITS: 0` and `RAW_RED_HITS: 0` and `BACKDROP_BLUR_HITS: 0`
14. `CONSOLE_NEW_ERRORS: 0`
15. `## settings — audit` and `## settings — critique` headings appended (printed to chat from the file)
