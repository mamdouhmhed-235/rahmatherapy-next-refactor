...
- Append `step-0: COMPLETE — skills verified` to `/redesign/per-page-progress/privacy-progress.md` and cat it.
...
Use `run_in_background: true`. Poll `http://localhost:3019/admin/privacy` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/privacy-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during the status-change flow — verify POST to `updatePrivacyRequestStatus` server action (one POST per individual transition)
...
9. `DEV_SERVER_READY at http://localhost:3019`
...
15. `## privacy — audit` and `## privacy — critique` headings appended (printed to chat from the file)
