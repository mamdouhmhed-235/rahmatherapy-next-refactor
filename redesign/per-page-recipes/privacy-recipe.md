...
```
/ralph-loop "Read the brief at /redesign/briefs/privacy-brief.md. Compare the current implementation to the brief's requirements (5 native sections + Recipe Context + Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
Use `run_in_background: true`. Poll `http://localhost:3001/admin/privacy` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 60 seconds.
...
**Evidence to surface:**
- 5+ screenshot file paths printed to chat (`ls redesign/screenshots/privacy-redesign/`)
- Literal decision: `ITERATE_DECISION: no refine needed — <brief-justified reason>` OR `ITERATE_DECISION: ran /impeccable <axis> privacy because <reason>`
...
grep -nE '\\d+px' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx
...
grep -nE '(margin\|padding):\s*\d' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx
...
- Print network requests during the status-change flow — verify POST to `updatePrivacyRequestStatus` server action (one POST per individual transition)
...
## Step 12 — `/impeccable audit privacy` + `/impeccable critique privacy` + functional smoke test

### 12a — Audit
Invoke Skill with `/impeccable audit privacy`.
...
Append to `/redesign/PER-PAGE-SCORES.md` under heading `## privacy — audit`:
- 5 dimension scores
- P0/P1/P2/P3 findings, each on its own line
- Backend status: `FAKE — BUILD-privacy-filter-query.md (BLOCKS-REDESIGN, Layer 0 row 11)`
...
### 12b — Critique
Invoke Skill with `/impeccable critique privacy`.

Append to `/redesign/PER-PAGE-SCORES.md` under heading `## privacy — critique`:
...
If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder privacy` OR `/impeccable distill privacy` based on which fits the verdict's reasoning, then re-run `/impeccable critique privacy`. Loop max 2 times.
...
2. Compare changed-files list against `/redesign/per-page-scope/privacy-scope.md`. For any file changed that isn't in the scope file's "Files to edit" list: STOP and emit `SCOPE_VIOLATION: <file>`. Otherwise: `SCOPE_CLEAN: only scoped files changed`.
...
4. Emit the handoff message to chat with:
   - Dev server URL: `http://localhost:3001/admin/privacy`
   - All screenshot paths
   - Audit + critique key scores
   - Backend FAKE flag: filter-strip query still depends on `BUILD-privacy-filter-query.md`
...
13. `## privacy — audit` and `## privacy — critique` headings appended (printed to chat from the file)
