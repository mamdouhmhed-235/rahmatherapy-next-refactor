# /goal recipe — page: privacy (25 of 29)

> **You are executing this recipe under a `/goal` session.** Follow every step in order. After each step, append a `step-N: COMPLETE — <one-line summary>` line to `/redesign/per-page-progress/privacy-progress.md` and `cat` the full file to chat. The Haiku evaluator will read your transcript after every turn — it can only see what you surface. Print evidence to chat liberally.
...
|---|---|
| Page slug | `privacy` |
| Page row in IMPLEMENTATION-PLAN.md | row 25 of 29 (renumber based on chronological position, not numeric label) |
| Brief | `/redesign/briefs/privacy-brief.md` |
| Workflow guide reference | `/redesign/phase6-admin-workflow-guide.html` (privacy section) |
| Source files to edit | `src/app/admin/privacy/page.tsx`, `src/app/admin/privacy/PrivacyStatusForm.tsx` (and any new client wrapper introduced during craft — keep new files under `src/app/admin/privacy/`) |
| Worktree | this checkout — branch `agent/privacy-redesign` off `redesign/start-state` |
...
| Dev server port for this worktree | `3019` (user's main tree owns `3000`) |
| node_modules | already junctioned from main tree to this worktree's `./node_modules` — do not reinstall |
| Backend status | `FAKE` — `BUILD-privacy-filter-query.md` is BLOCKS-REDESIGN (IMPLEMENTATION-PLAN.md row L0/11). Until handled, the filter strip renders and the URL persists but server returns the unfiltered page-load result; flag FAKE in the audit. |
| Progress scratchpad | `/redesign/per-page-progress/privacy-progress.md` |
...
5. **Preserve form `name` attributes:** `name="request_id"` and `name="status"` (hidden) on every `PrivacyStatusForm` must remain literal and verbatim (RECON §6.4).
6. **Preserve the server action contract:** every status form must keep calling `updatePrivacyRequestStatus`. No `fetch` / no `XHR` replacement. The four-status enum (Received / Reviewing / Completed / Declined) is fixed.
...
1. The brief at `/redesign/briefs/privacy-brief.md` — quote the relevant section verbatim.
...
1. Append to `/redesign/per-page-deferrals/privacy-deferrals.md` in this format:
...
- `redesign/RECIPE-PROGRESS.md` (~26k tokens) → read lines 1–100 for overview; grep for `Phase 6` and `privacy` for page-specific entries
- `redesign/IMPLEMENTATION-PLAN.md` (~29k tokens) → read lines 1–60 for "Currently on"; lines 925–965 for the privacy row
...
>
> CONFIRMATION CHALLENGE: after your one-sentence-per-file summary, quote the SECOND sentence of `/redesign/briefs/privacy-brief.md`'s `## 6. Key States` section verbatim. If you can't quote it accurately, you didn't actually re-read — re-read from disk now and try again. This guards against summarised cache.
...
> - /redesign/briefs/privacy-brief.md  ← THIS IS THE PREPARED BRIEF
>
...
>
> IMAGE HANDLING: `privacy-empty.svg` (clipboard-with-shield) is listed in IMAGES-NEEDED for the page-level EmptyState. If it isn't on disk yet, mark the slot with `data-redesign-needs-photo` and degrade gracefully to the EmptyState's text-only fallback.
>
> BACKEND FAKE MARKER: filter strip submits via GET with `request_type`, `status`, `from`, `to`, `q`, plus optional `sort=created_at_asc` and `expand=all`. The server-side filter query is still un-implemented per `BUILD-privacy-filter-query.md` (BLOCKS-REDESIGN). Render the filter strip as if it works, but add `data-redesign-fake="filter-query"` on the form element. Stat-tile click-to-filter falls under the same FAKE umbrella.

**Evidence to surface:**
- Literal line `SCOPE_PROPOSAL:` followed by the files-to-edit list
- `/redesign/per-page-scope/privacy-scope.md` written; print its contents to chat
...
## Step 4 — `/impeccable craft redesign of admin page privacy`

**Action:** Invoke Skill tool with `/impeccable craft redesign of admin page privacy`. Feed the brief sections verbatim to shape discovery as they're requested. Let craft do its full build phase.
...
```
/ralph-loop "Read the brief at /redesign/briefs/privacy-brief.md. Compare the current implementation to the brief's requirements (every section of the brief, including Recipe Context and Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
## Step 6 — Start dev server on port 3019 (in this worktree)

**Action:** node_modules is already junctioned. Start the dev server in the background:

**Pre-flight (do this BEFORE the cd):** verify the worktree directory exists. If `Test-Path` (PowerShell) or `[ -d ... ]` (bash) returns false on `C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-privacy-redesign`, emit `STUCK: 6 — worktree directory missing — re-run the worktree setup from LAUNCH-SHEET Section 1a` and STOP. Do not try to recreate the worktree from inside the agent.

```bash
cd "C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-privacy-redesign"
pnpm next dev -p 3019
```

Use `run_in_background: true`. Poll `http://localhost:3019/admin/privacy` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree).

**Evidence to surface:**
- The HTTP status code from the readiness poll printed to chat
- Literal line `DEV_SERVER_READY at http://localhost:3019`
- Append `step-6: COMPLETE — dev server on 3019` and cat progress file

---

## Step 7 — Step 2 iterate (screenshots + multi-axis polish, max 4 axes)

**Action:** Use the `playwright` MCP tool. Sign in first with `test.admin@rahmatherapy.example.test` / `AdminTest123!`. Note: by default PM holds neither privacy permission (brief §11), so the admin seed may land on `AdminAccessDenied`. If that happens, swap to the owner test account documented in `/redesign/test-credentials.md` (Owner holds both `manage_privacy_operations` and `manage_sensitive_client_notes` plus `view_client_contact_details`).

Take screenshots and save to `/redesign/screenshots/privacy-redesign/`:
- `chunk1-1440-default.png` at 1440×900
- `chunk1-768-default.png` at 768×1024
- `chunk1-375-default.png` at 375×812
- `chunk1-1440-expand-all.png` at 1440×900 navigating to `/admin/privacy?expand=all`
...
| Symptom | Skill |
|---|---|
| Generic, like every SaaS | `/impeccable bolder privacy` |
| Too loud, too many colours | `/impeccable quieter privacy` |
| Grey, lifeless, no identity | `/impeccable colorize privacy` |
| Fonts feel default or inconsistent | `/impeccable typeset privacy` |
| Spacing is off, things feel cramped | `/impeccable layout privacy` |
| Static, jumpy, no motion | `/impeccable animate privacy` |
| Functional but cold | `/impeccable delight privacy` |
| Too much on the page | `/impeccable distill privacy` |

**Skip `/impeccable live`** — interactive only, doesn't work headless.

**For each chosen axis (sequential, not parallel):**
1. Invoke the impeccable Skill tool with `<axis> privacy` args. Use the Skill tool (not the slash-command shorthand) so the invocation appears as a transcript event the Haiku evaluator can see.
2. After it completes, take `chunk1-1440-after-<axis>.png` at 1440×900 and save to `/redesign/screenshots/privacy-redesign/`.
...
After all axes complete, take post-polish screenshots at all 3 viewports: `privacy-post-axes-{375,768,1440}.png` to `/redesign/screenshots/privacy-redesign/`.

**Evidence to surface:**
- All baseline + per-axis + post-polish screenshot file paths printed to chat (`ls redesign/screenshots/privacy-redesign/`)
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/privacy-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Final 3-viewport screenshots: `privacy-polish-final-{375,768,1440}.png` saved to `/redesign/screenshots/privacy-redesign/`
- Append `step-7b: COMPLETE — polish loop done` and cat progress file

---

## Step 8 — `/impeccable adapt privacy for mobile and tablet`

**Action:** Invoke Skill with `/impeccable adapt privacy for mobile and tablet`.

After adapt finishes:
1. Screenshot at 768×1024 → save to `/redesign/baseline/privacy-adapt-after-tablet.png`
2. Screenshot at 375×812 → save to `/redesign/baseline/privacy-adapt-after-mobile.png`
...
4. Confirm `<details>` "Update status" disclosure on each request row is tappable without zoom on mobile (min 44px touch targets — check `getBoundingClientRect().height >= 44`)
5. Confirm sensitive-note rail collapses below the request queue on `<lg:` as a collapsed-by-default `<details>` "Sensitive notes (25)" per brief §5

**Evidence to surface:**
- Two `privacy-adapt-after-*.png` paths printed
- Horizontal-scroll check result for both viewports (`HORIZONTAL_SCROLL_TABLET: false`, `HORIZONTAL_SCROLL_MOBILE: false`)
- Touch-target height for `<details>` "Update status" trigger on mobile (`TOUCH_TARGET_STATUS_DISCLOSURE_MOBILE: <px>`)
- Append `step-8: COMPLETE — adapt run, breakpoints clean` and cat progress file

---

## Step 9 — `/impeccable harden privacy`

**Action:** Invoke Skill with `/impeccable harden privacy`.

Save the FULL harden recommendations report to `/redesign/HARDEN-RECS-privacy.md`. Implement what harden recommends (empty / loading / error / permission-denied / overflow states) per the brief's `## 6. Key States` section. For each state listed in the brief that harden didn't surface, add it anyway. Pay particular attention to the Layer-3 backend error table at the bottom of brief §6 (initial-load failure, contact details absent for `canViewClientContactDetails`-lacking caller, filtered-empty per-section behaviour).

Verification edge cases (privacy-specific):
- A request_note with 1200 characters wraps in the Soft Slate well at 375px without breaking the row; "Show more" toggles
- The "Sensitive notes" rail with 25 rows scrolls within its sticky panel without leaking outside on tall viewports
- Mark-completed `ConfirmActionModal` body wraps clean at 375px with the full legal-posture sentence
- Contact-detail line silently omits (no "hidden" placeholder copy) when caller lacks `view_client_contact_details`
- The `expand=all` URL param overrides the default-collapse for Completed and Declined panels

**Evidence to surface:**
- `/redesign/HARDEN-RECS-privacy.md` exists; print first 60 lines to chat
- List of states added (or "none, harden confirmed existing states cover brief")
- File paths touched (`git diff --stat`)
- Append `step-9: COMPLETE — harden run, HARDEN-RECS-privacy.md saved` and cat progress file

---

## Step 10 — `/impeccable clarify privacy`

**Action:** Invoke Skill with `/impeccable clarify privacy`.
...
- No `Submit` buttons remain → `Save status`, `Mark completed`, `Decline`, `Cancel`, `Apply filters`, `Clear filters`, `Open client`, `Show more`/`Show less`, `Show {N}`/`Hide {N}`
- No `An error occurred` messages → use brief's specific copy (e.g. `Couldn't load privacy requests. Try refreshing.`)
- Toast copy verbatim: `Request marked {status}.` / `Couldn't update the request. Try again.`
- Empty-state heads + bodies match the brief's table (page-level "No privacy requests yet" / per-section "No {status} requests" inline)
- Confirm-modal copy verbatim: "Mark request as completed?" body mentions booking + audit-log integrity; "Decline this request?" body mentions ICO escalation
- Denied screen reads "Privacy operations restricted" with no raw `manage_privacy_operations or manage_sensitive_client_notes` identifier
...
grep -nE '#[0-9a-fA-F]{3,8}' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx

# Raw oklch() literals (should be 0 — colors come from tokens)
grep -nE 'oklch\(' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx

# Raw px values outside @media queries (canon: should be 0 outside @media rules)
grep -nE '[0-9]+px' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx | grep -v '@media'

# font-family literals (should be 0)
grep -nE "font-family:\s*['\"]" src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx

# Raw spacing literals (canon: should match the spacing scale in DESIGN.md)
grep -nE '(margin|padding):[[:space:]]*[0-9]' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx

# legacy raw rahma escapes (Phase-6 soft fix per brief §4)
grep -nE 'var\(--rahma-' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx

# raw bg-white on the request article (page.tsx:180 carry-forward)
grep -n 'bg-white' src/app/admin/privacy/page.tsx src/app/admin/privacy/PrivacyStatusForm.tsx
```

For each match, confirm the value comes from a DESIGN.md token. If a hardcoded value isn't backed by a token, FIX IT — either add the token to DESIGN.md (only with user approval, otherwise STUCK) or replace with the existing token. Status-family backgrounds must come from `var(--admin-attention)` / `var(--admin-pending)` / `var(--admin-confirmed)` / `var(--admin-cancelled)`.

### 11b — Playwright verification at all viewports (mobile-first: 375 → 768 → 1440)

- Sign in first at `/admin/login` (owner credentials likely needed; brief §11 makes PM denied by default).
- Navigate to `/admin/privacy` at each viewport
- Save final-state screenshots: `privacy-final-{375,768,1440}.png` to `/redesign/screenshots/privacy-redesign/`
- Exercise the primary action: open a Received request's `<details>` "Update status" disclosure → change status to `Reviewing` → click `Save status` → verify the row migrates from Received to Reviewing panel without a full reload; verify Sonner `Request marked reviewing.`
- Exercise a destructive transition: open a Reviewing row → set status `Completed` → click `Save status` → verify `ConfirmActionModal` fires with the brief §6 body referencing booking + audit-log integrity → click `Mark completed` (Primary) → verify success Sonner `Request marked completed.`
- Take a verification screenshot of the success toast: `privacy-complete-toast.png`
...
- Use the chrome-devtools MCP to inspect network requests during the status-change flow — verify POST to `updatePrivacyRequestStatus` server action (one POST per individual transition)

**Evidence to surface:**
- All grep results in chat with explicit `TOKEN_DRIFT: 0` (or list each match + fix)
- Screenshot files in `/redesign/screenshots/privacy-redesign/`: `privacy-final-{375,768,1440}.png` + `privacy-complete-toast.png`
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (the slug `privacy` is already substituted below — pass this prompt verbatim):

```
You are auditing the redesign of admin page privacy for Phase 6 of the Rahma Therapy admin redesign. The page has just been crafted, polished, adapted, and hardened by another agent. Your job is an objective code + design audit — you have NO bias from doing the work.

Re-prime (read these in order, in full):
1. /redesign/briefs/privacy-brief.md
2. PRODUCT.md
3. DESIGN.md (full, including ## Admin-Specific Patterns)
4. /redesign/IMPLEMENTATION-PLAN.md — find the privacy row to determine Backend status (N-A / FAKE / HANDLED) and any BUILD plan dependencies
5. /redesign/BUSINESS-COMPLETENESS.md — to identify any Track A items this page contributes to
6. The post-polish screenshots at /redesign/screenshots/privacy-redesign/privacy-polish-final-{375,768,1440}.png
7. The current source code: src/app/admin/privacy/** and any other files in the recipe's "Files to edit" list
...
Task: invoke the impeccable Skill with `audit privacy`. Score 5 dimensions and surface all P0/P1/P2/P3 findings with file:line references.

Return format — the full audit text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## privacy — audit`, with these required subsections:
- 5 dimension scores
...
2. Append it verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## privacy — audit`.
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (`privacy` already substituted):

```
You are critiquing the redesign of admin page privacy for Phase 6. The page has been crafted + polished + adapted + hardened + audited by another agent. Your job is an objective UX critique — you have NO bias from doing the work.

Re-prime (read in full):
1. /redesign/briefs/privacy-brief.md
2. PRODUCT.md
3. DESIGN.md (full)
4. The post-polish screenshots at /redesign/screenshots/privacy-redesign/privacy-polish-final-{375,768,1440}.png
5. The current source code: src/app/admin/privacy/**

Task: invoke the impeccable Skill with `critique privacy`. Return:
...
Return format — the full critique text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## privacy — critique`.

Do NOT write to PER-PAGE-SCORES.md. Return the full critique text verbatim.
```

After the subagent returns:
1. Append verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## privacy — critique`.
2. **Print to chat verbatim** — same reasoning as 12a.
3. If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder privacy` or `/impeccable distill privacy` (whichever fits the verdict's reasoning), then re-dispatch the critique subagent with the same prompt. Loop max 2 times. If after 2 loops the verdict is still REGRESSED/FAIL, append the verdict + reasoning to `/redesign/per-page-deferrals/privacy-deferrals.md` with **Defer to: Phase 7** and proceed to 12c.

### 12c — Functional smoke test

Run through brief's Feature Preservation Manifest manually via Playwright + read-the-code:

- [ ] `PrivacyStatusForm` `<form action={updatePrivacyRequestStatus}>` with hidden `request_id` + `status` inputs still present and functional on every row
- [ ] Status transitions to Received / Reviewing are one-click; transitions to Completed / Declined gate through `ConfirmActionModal` with the brief's legal-posture copy
- [ ] Stat-tile click → "Open requests" applies `status=received,reviewing`; "Awaiting longest" applies `status=received&sort=created_at_asc` and scrolls to the oldest row
- [ ] Sensitive-note rail loads 25 rows; client name link routes to `/admin/clients/<id>`; "Open client →" Ghost link present per row
- [ ] Contact-detail line silently absent when caller lacks `view_client_contact_details` (no "hidden" placeholder copy)
- [ ] `expand=all` URL param overrides default-collapse for Completed and Declined panels
- [ ] Four status panels render in fixed order; Received + Reviewing expanded; Completed + Declined collapsed by default
- [ ] `AdminAccessDenied` does NOT render the raw `manage_privacy_operations or manage_sensitive_client_notes` identifier
...
- [ ] PER-PAGE-SCORES.md sections appended (`## privacy — audit` + `## privacy — critique`) and printed to chat verbatim from the subagent results (Step 12a + 12b).
- [ ] Deferral file written at `/redesign/per-page-deferrals/privacy-deferrals.md` — even if empty, write `(no deferrals — Phase 6 closed cleanly for privacy)`. The main agent and the Phase 7 gauntlet both read this; missing file = ambiguous closure.
...
- Dev server URL: `http://localhost:3019/admin/privacy`
...
8. `DEV_SERVER_READY at http://localhost:3019`
...
14. `## privacy — audit` and `## privacy — critique` headings appended (printed to chat from the file)
