...
| Dev server port for this worktree | `3027` (user's main tree owns `3000`) |
...
1. The brief at `/redesign/briefs/staff-detail-brief.md` — quote the relevant section verbatim.
...
1. Append to `/redesign/per-page-deferrals/staff-detail-deferrals.md` in this format:
...
```
/ralph-loop "Read the brief at /redesign/briefs/staff-detail-brief.md. Compare the current implementation to the brief's requirements (every section of the brief, including Recipe Context and Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
## Step 6 — Start dev server on port 3027 (in this worktree)

**Action:** node_modules is already junctioned. Start the dev server in the background:

**Pre-flight (do this BEFORE the cd):** verify the worktree directory exists. If `Test-Path` (PowerShell) or `[ -d ... ]` (bash) returns false on `C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-staff-detail-redesign`, emit `STUCK: 6 — worktree directory missing — re-run the worktree setup from LAUNCH-SHEET Section 1a` and STOP. Do not try to recreate the worktree from inside the agent.

```bash
cd "C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-staff-detail-redesign"
pnpm next dev -p 3027
```

Use `run_in_background: true`. Poll `http://localhost:3027/admin/staff` until it returns HTTP 200 (or 308). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree). Then resolve a seed staff UUID for use in subsequent steps (read the seed file or query the test admin's own staff_id).
...
- Literal line `DEV_SERVER_READY at http://localhost:3027`
- Append `step-6: COMPLETE — dev server on 3027` and cat progress file
...
1. Invoke the impeccable Skill tool with `<axis> staff-detail` args. Use the Skill tool (not the slash-command shorthand) so the invocation appears as a transcript event the Haiku evaluator can see.
2. After it completes, take `chunk1-1440-after-<axis>.png` at 1440×900 and save to `/redesign/screenshots/staff-detail-redesign/`.
...
After all axes complete, take post-polish screenshots at all 3 viewports: `staff-detail-post-axes-{375,768,1440}.png` to `/redesign/screenshots/staff-detail-redesign/`.

**Evidence to surface:**
- All baseline + per-axis + post-polish screenshot file paths printed to chat (`ls redesign/screenshots/staff-detail-redesign/`)
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/staff-detail-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Final 3-viewport screenshots: `staff-detail-polish-final-{375,768,1440}.png` saved to `/redesign/screenshots/staff-detail-redesign/`
...
grep -nE '[0-9]+px' src/app/admin/staff/\[staffId\]/*.tsx | grep -v '@media'
...
grep -nE '(margin|padding):[[:space:]]*[0-9]' src/app/admin/staff/\[staffId\]/*.tsx
...
- Use the chrome-devtools MCP to inspect network requests during profile save — verify the POST to the `StaffProfileForm` server action with all preserved field names
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (the slug `staff-detail` is already substituted below — pass this prompt verbatim):

```
You are auditing the redesign of admin page staff-detail for Phase 6 of the Rahma Therapy admin redesign. The page has just been crafted, polished, adapted, and hardened by another agent. Your job is an objective code + design audit — you have NO bias from doing the work.

Re-prime (read these in order, in full):
1. /redesign/briefs/staff-detail-brief.md
2. PRODUCT.md
3. DESIGN.md (full, including ## Admin-Specific Patterns)
4. /redesign/IMPLEMENTATION-PLAN.md — find the staff-detail row to determine Backend status (N-A / FAKE / HANDLED) and any BUILD plan dependencies
5. /redesign/BUSINESS-COMPLETENESS.md — to identify any Track A items this page contributes to
6. The post-polish screenshots at /redesign/screenshots/staff-detail-redesign/staff-detail-polish-final-{375,768,1440}.png
7. The current source code: src/app/admin/staff-detail/** and any other files in the recipe's "Files to edit" list
...
Task: invoke the impeccable Skill with `audit staff-detail`. Score 5 dimensions and surface all P0/P1/P2/P3 findings with file:line references.

Return format — the full audit text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## staff-detail — audit`, with these required subsections:
- 5 dimension scores
...
2. Append it verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## staff-detail — audit`.
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (`staff-detail` already substituted):

```
You are critiquing the redesign of admin page staff-detail for Phase 6. The page has been crafted + polished + adapted + hardened + audited by another agent. Your job is an objective UX critique — you have NO bias from doing the work.

Re-prime (read in full):
1. /redesign/briefs/staff-detail-brief.md
2. PRODUCT.md
3. DESIGN.md (full)
4. The post-polish screenshots at /redesign/screenshots/staff-detail-redesign/staff-detail-polish-final-{375,768,1440}.png
5. The current source code: src/app/admin/staff-detail/**

Task: invoke the impeccable Skill with `critique staff-detail`. Return:
...
Return format — the full critique text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## staff-detail — critique`.

Do NOT write to PER-PAGE-SCORES.md. Return the full critique text verbatim.
```

After the subagent returns:
1. Append verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## staff-detail — critique`.
2. **Print to chat verbatim** — same reasoning as 12a.
3. If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder staff-detail` or `/impeccable distill staff-detail` (whichever fits the verdict's reasoning), then re-dispatch the critique subagent with the same prompt. Loop max 2 times. If after 2 loops the verdict is still REGRESSED/FAIL, append the verdict + reasoning to `/redesign/per-page-deferrals/staff-detail-deferrals.md` with **Defer to: Phase 7** and proceed to 12c.
...
- [ ] PER-PAGE-SCORES.md sections appended (`## staff-detail — audit` + `## staff-detail — critique`) and printed to chat verbatim from the subagent results (Step 12a + 12b).
- [ ] Deferral file written at `/redesign/per-page-deferrals/staff-detail-deferrals.md` — even if empty, write `(no deferrals — Phase 6 closed cleanly for staff-detail)`. The main agent and the Phase 7 gauntlet both read this; missing file = ambiguous closure.
...
- Dev server URL: `http://localhost:3027/admin/staff-detail`
...
7. `DEV_SERVER_READY at http://localhost:3027`
