...
Use `run_in_background: true`. Poll `http://localhost:3021/admin/roles` (the list, then click a row) until it returns HTTP 200 (or 308). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree). The detail route is `/admin/roles/<roleId>` — pick an existing role from the seed (e.g. the Owner or Booking Coordinator role); resolve `<roleId>` by reading the seed migrations or by signing in and clicking through.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/role-detail-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use the chrome-devtools MCP to inspect network requests during the permission-toggle + metadata-save flows — verify POST to `toggleRolePermission` server action per toggle and POST to `updateRoleMetadata` on metadata save
...
8. `DEV_SERVER_READY at http://localhost:3021`
...
14. `## role-detail — audit` and `## role-detail — critique` headings appended (printed to chat from the file)
