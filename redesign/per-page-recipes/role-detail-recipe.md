# /goal recipe — page: role-detail (26 of 29)

> **You are executing this recipe under a `/goal` session.** Follow every step in order. After each step, append a `step-N: COMPLETE — <one-line summary>` line to `/redesign/per-page-progress/role-detail-progress.md` and `cat` the full file to chat. The Haiku evaluator will read your transcript after every turn — it can only see what you surface. Print evidence to chat liberally.
...
| Page row in IMPLEMENTATION-PLAN.md | row 26 of 29 (renumber based on chronological position, not numeric label) |
| Brief | `/redesign/briefs/role-detail-brief.md` |
| Workflow guide reference | `/redesign/phase6-admin-workflow-guide.html` (role-detail section) |
| Source files to edit | `src/app/admin/roles/[roleId]/page.tsx`, `src/app/admin/roles/[roleId]/RoleMetadataForm.tsx`, `src/app/admin/roles/[roleId]/PermissionRow.tsx` (and any new client wrappers introduced during craft for the danger-zone / confirm flows — keep new files under `src/app/admin/roles/[roleId]/`) |
| Worktree | this checkout — branch `agent/role-detail-redesign` off `redesign/start-state` |
...
| Dev server port for this worktree | `3021` (user's main tree owns `3000`) |
| node_modules | already junctioned from main tree to this worktree's `./node_modules` — do not reinstall |
| Backend status | `FAKE` — `BUILD-delete-role.md` is non-blocking (IMPLEMENTATION-PLAN.md L0/24). Until handled, the danger-zone "Delete role" button renders but its server action is absent; degrade gracefully (button disabled with inline note, or surface server error toast). High/critical/owner confirmation modals work today; only `deleteRole` is the new server action. |
| RBAC scope | Owner only (`manage_role_templates` is owner-exclusive). Test creds must be the owner account, not the admin seed. |
| Progress scratchpad | `/redesign/per-page-progress/role-detail-progress.md` |
...
5. **Preserve form `name` attributes:** `RoleMetadataForm` keeps `role_id`, `display_label`, `description`, `sort_order`, `active` plus the hidden `active=on` shadow for system roles (RECON §6.4). `PermissionRow` keeps the `toggleRolePermission(roleId, permissionId, permissionName)` server-action signature.
6. **Preserve the server action contracts:** `<Switch>` on each permission row stays wired to `toggleRolePermission`; metadata form stays wired to `updateRoleMetadata`. No `fetch` / no `XHR` replacement. `deleteRole` is net-new per §10 Q1; if the BUILD plan hasn't landed, leave the Delete button visually present but disabled with the brief's "System roles can't be deleted." / "Reassign {N} staff first" tooltips, or hide the button entirely — pick the gentler degrade and surface the choice.
...
1. The brief at `/redesign/briefs/role-detail-brief.md` — quote the relevant section verbatim.
...
1. Append to `/redesign/per-page-deferrals/role-detail-deferrals.md` in this format:
...
- `redesign/RECIPE-PROGRESS.md` (~26k tokens) → read lines 1–100 for overview; grep for `Phase 6` and `role-detail` for page-specific entries
- `redesign/IMPLEMENTATION-PLAN.md` (~29k tokens) → read lines 1–60 for "Currently on"; lines 965–1005 for the role-detail row
...
- Append `step-0: COMPLETE — skills verified` to `/redesign/per-page-progress/role-detail-progress.md` and cat it.
...
>
> CONFIRMATION CHALLENGE: after your one-sentence-per-file summary, quote the SECOND sentence of `/redesign/briefs/role-detail-brief.md`'s `## 6. Key States` section verbatim. If you can't quote it accurately, you didn't actually re-read — re-read from disk now and try again. This guards against summarised cache.
...
> - /redesign/briefs/role-detail-brief.md  ← THIS IS THE PREPARED BRIEF
>
...
>
> IMAGE HANDLING: brief lists no net-new SVG (role-letter token uses typography). No `data-redesign-needs-photo` markers required.
>
> BACKEND FAKE MARKER: `deleteRole(roleId)` is a net-new server action per brief §10 Q1, tracked at `BUILD-delete-role.md` (non-blocking). Until handled, the Delete-role button in the danger-zone panel should either be hidden behind a guard (`!is_system && staff_count === 0 && hasDeleteRoleAction`) or rendered disabled with the brief's inline note. Add `data-redesign-fake="delete-role"` on the button while in the degraded path. The high-risk, critical-risk, Owner, and Deactivate `ConfirmActionModal` flows are NOT fake — they wire into existing actions.

**Evidence to surface:**
- Literal line `SCOPE_PROPOSAL:` followed by the files-to-edit list
- `/redesign/per-page-scope/role-detail-scope.md` written; print its contents to chat
...
## Step 4 — `/impeccable craft redesign of admin page role-detail`

**Action:** Invoke Skill tool with `/impeccable craft redesign of admin page role-detail`. Feed the brief sections verbatim to shape discovery as they're requested. Let craft do its full build phase.
...
```
/ralph-loop "Read the brief at /redesign/briefs/role-detail-brief.md. Compare the current implementation to the brief's requirements (every section of the brief, including Recipe Context and Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
## Step 6 — Start dev server on port 3021 (in this worktree)

**Action:** node_modules is already junctioned. Start the dev server in the background:

**Pre-flight (do this BEFORE the cd):** verify the worktree directory exists. If `Test-Path` (PowerShell) or `[ -d ... ]` (bash) returns false on `C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-role-detail-redesign`, emit `STUCK: 6 — worktree directory missing — re-run the worktree setup from LAUNCH-SHEET Section 1a` and STOP. Do not try to recreate the worktree from inside the agent.

```bash
cd "C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-role-detail-redesign"
pnpm next dev -p 3021
```

Use `run_in_background: true`. Poll `http://localhost:3021/admin/roles` (the list, then click a row) until it returns HTTP 200 (or 308). Max wait: 60 seconds. The detail route is `/admin/roles/<roleId>` — pick an existing role from the seed (e.g. the Owner or Booking Coordinator role); resolve `<roleId>` by reading the seed migrations or by signing in and clicking through.
...
- The resolved `<roleId>` value (UUID or slug) printed: `RESOLVED_ROLE_ID: <id>`
- Literal line `DEV_SERVER_READY at http://localhost:3021`
- Append `step-6: COMPLETE — dev server on 3021` and cat progress file

---

## Step 7 — Step 2 iterate (screenshots + multi-axis polish, max 4 axes)

**Action:** Use the `playwright` MCP tool. Sign in first with the owner test account (role-detail is owner-only per brief §11; `test.admin@rahmatherapy.example.test` will hit `AdminAccessDenied`). Resolve owner credentials from `/redesign/test-credentials.md`.

Take screenshots and save to `/redesign/screenshots/role-detail-redesign/`:
- `chunk1-1440-default.png` at 1440×900 (a non-Owner non-system custom role, ideally with `staff_count > 0`)
- `chunk1-1440-owner-role.png` at 1440×900 (the Owner role specifically — to show the Owner-role permission-toggle confirm path)
- `chunk1-1440-system-role.png` at 1440×900 (a `is_system=true` role — to show Delete-role hidden + system note)
...
| Symptom | Skill |
|---|---|
| Generic, like every SaaS | `/impeccable bolder role-detail` |
| Too loud, too many colours | `/impeccable quieter role-detail` |
| Grey, lifeless, no identity | `/impeccable colorize role-detail` |
| Fonts feel default or inconsistent | `/impeccable typeset role-detail` |
| Spacing is off, things feel cramped | `/impeccable layout role-detail` |
| Static, jumpy, no motion | `/impeccable animate role-detail` |
| Functional but cold | `/impeccable delight role-detail` |
| Too much on the page | `/impeccable distill role-detail` |

**Skip `/impeccable live`** — interactive only, doesn't work headless.

**For each chosen axis (sequential, not parallel):**
1. Invoke the impeccable Skill tool with `<axis> role-detail` args. Use the Skill tool (not the slash-command shorthand) so the invocation appears as a transcript event the Haiku evaluator can see.
2. After it completes, take `chunk1-1440-after-<axis>.png` at 1440×900 and save to `/redesign/screenshots/role-detail-redesign/`.
...
After all axes complete, take post-polish screenshots at all 3 viewports: `role-detail-post-axes-{375,768,1440}.png` to `/redesign/screenshots/role-detail-redesign/`.

**Evidence to surface:**
- All baseline + per-axis + post-polish screenshot file paths printed to chat (`ls redesign/screenshots/role-detail-redesign/`)
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/role-detail-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Final 3-viewport screenshots: `role-detail-polish-final-{375,768,1440}.png` saved to `/redesign/screenshots/role-detail-redesign/`
- Append `step-7b: COMPLETE — polish loop done` and cat progress file

---

## Step 8 — `/impeccable adapt role-detail for mobile and tablet`

**Action:** Invoke Skill with `/impeccable adapt role-detail for mobile and tablet`.

After adapt finishes:
1. Screenshot at 768×1024 → save to `/redesign/baseline/role-detail-adapt-after-tablet.png`
2. Screenshot at 375×812 → save to `/redesign/baseline/role-detail-adapt-after-mobile.png`
...
4. Confirm permission-row Switch is tappable without zoom on mobile (min 44px touch targets — check `getBoundingClientRect().height >= 44`)
5. Confirm sidebar collapses below permissions on `<lg:` in the order Metadata → Staff → Danger zone (brief §5 Mobile)
6. Confirm sticky category headers `position: sticky` disable on mobile per brief §10 Q3 (categories scroll with the page)

**Evidence to surface:**
- Two `role-detail-adapt-after-*.png` paths printed
- Horizontal-scroll check result for both viewports (`HORIZONTAL_SCROLL_TABLET: false`, `HORIZONTAL_SCROLL_MOBILE: false`)
- Touch-target height for permission Switch on mobile (`TOUCH_TARGET_SWITCH_MOBILE: <px>`)
- Append `step-8: COMPLETE — adapt run, breakpoints clean` and cat progress file

---

## Step 9 — `/impeccable harden role-detail`

**Action:** Invoke Skill with `/impeccable harden role-detail`.

Save the FULL harden recommendations report to `/redesign/HARDEN-RECS-role-detail.md`. Implement what harden recommends (empty / loading / error / permission-denied / overflow states) per the brief's `## 6. Key States` section. Pay particular attention to the Layer-3 backend error table at the bottom of brief §6 (delete race with newly-assigned staff, `is_system` server-side bypass, DB constraint failure on delete).

Verification edge cases (role-detail-specific):
- A role with 47 permissions across 9 categories renders without breaking sticky headers
- The Owner role's "Change Owner permissions?" modal body wraps clean at 375px
- A `risk_level=critical` permission's grant + revoke confirm modal bodies differ (brief §6: grant talks about "expands authority"; revoke talks about "may prevent workflows")
- Staff sidebar with 12 entries renders without the panel scrolling off-page
- Description "Show more" `<details>` works when the description is exactly 2 lines (no toggle needed) and when it's 8 lines

**Evidence to surface:**
- `/redesign/HARDEN-RECS-role-detail.md` exists; print first 60 lines to chat
- List of states added (or "none, harden confirmed existing states cover brief")
- File paths touched (`git diff --stat`)
- Append `step-9: COMPLETE — harden run, HARDEN-RECS-role-detail.md saved` and cat progress file

---

## Step 10 — `/impeccable clarify role-detail`

**Action:** Invoke Skill with `/impeccable clarify role-detail`.
...
- No `Submit` buttons remain → `Save role details`, `Apply filters`, `Clear filters`, `Show more`/`Show less`, `Deactivate role`, `Reactivate role`, `Delete role`, `Cancel`, `Change Owner permission`, `Grant`, `Revoke`, `Deactivate`
- Risk chip text verbatim: `Low risk` / `Medium risk` / `High risk` / `Critical risk`
- Scope chip text verbatim: `scope: {scope}`
- Permission toast copy verbatim: `Granted {permission display name}.` / `Revoked {permission display name}.`
- Metadata save toast: `Role saved.`
- Confirmation modal bodies match brief §6 verbatim (Owner / critical-grant / critical-revoke / high-grant / deactivate / delete)
- System-role inline note: `System roles can't be deleted.`
...
grep -nE '#[0-9a-fA-F]{3,8}' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx

# Raw oklch() literals (should be 0)
grep -nE 'oklch\(' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx

# Raw px values outside @media queries (canon: should be 0 outside @media rules)
grep -nE '[0-9]+px' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx | grep -v '@media'

# font-family literals (should be 0)
grep -nE "font-family:\s*['\"]" src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx

# Raw spacing literals (canon: should match the spacing scale in DESIGN.md)
grep -nE '(margin|padding):[[:space:]]*[0-9]' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx

# Legacy rahma escapes (Phase-6 soft fix per brief §4)
grep -nE 'var\(--rahma-' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx

# Inactive chip carry-forward (must retire bg-red-100 / text-red-700 in favour of Cancelled/Restricted family)
grep -nE 'bg-red-100|text-red-700|bg-white' src/app/admin/roles/\[roleId\]/page.tsx src/app/admin/roles/\[roleId\]/RoleMetadataForm.tsx src/app/admin/roles/\[roleId\]/PermissionRow.tsx
```

For each match, confirm the value comes from a DESIGN.md token. If a hardcoded value isn't backed by a token, FIX IT — either add the token to DESIGN.md (only with user approval, otherwise STUCK) or replace with the existing token. Risk chips must use `var(--admin-restricted)` / `var(--admin-pending)` / `var(--admin-attention)` / `var(--admin-cancelled)` families.

### 11b — Playwright verification at all viewports (mobile-first: 375 → 768 → 1440)

- Sign in first at `/admin/login` with owner credentials (role-detail is owner-only; admin seed will hit `AdminAccessDenied`).
- Navigate to `/admin/roles/<roleId>` at each viewport (use the role id resolved in Step 6)
- Save final-state screenshots: `role-detail-final-{375,768,1440}.png` to `/redesign/screenshots/role-detail-redesign/`
- Exercise the primary action: toggle a `risk_level=low` permission on → verify the Switch lands in the new state without a confirm modal; verify Sonner `Granted {permission display name}.`
- Toggle a `risk_level=high` permission on → verify `ConfirmActionModal` with brief §6 high-grant body fires → click `Grant` → verify success Sonner
- Toggle a `risk_level=critical` permission on → verify `ConfirmActionModal` with critical-grant body → click `Grant` → verify success Sonner
- Save metadata: edit `display_label`, click `Save role details` → verify Sonner `Role saved.` and page header reflects the new label without a full reload
- Take a verification screenshot of the critical-risk confirm modal: `role-detail-critical-modal.png`
...
- Use `chrome-devtools__list_network_requests` during the permission-toggle + metadata-save flows — verify POST to `toggleRolePermission` server action per toggle and POST to `updateRoleMetadata` on metadata save

**Evidence to surface:**
- All grep results in chat with explicit `TOKEN_DRIFT: 0` (or list each match + fix)
- Screenshot files in `/redesign/screenshots/role-detail-redesign/`: `role-detail-final-{375,768,1440}.png` + `role-detail-critical-modal.png`
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (the slug `role-detail` is already substituted below — pass this prompt verbatim):

```
You are auditing the redesign of admin page role-detail for Phase 6 of the Rahma Therapy admin redesign. The page has just been crafted, polished, adapted, and hardened by another agent. Your job is an objective code + design audit — you have NO bias from doing the work.

Re-prime (read these in order, in full):
1. /redesign/briefs/role-detail-brief.md
2. PRODUCT.md
3. DESIGN.md (full, including ## Admin-Specific Patterns)
4. /redesign/IMPLEMENTATION-PLAN.md — find the role-detail row to determine Backend status (N-A / FAKE / HANDLED) and any BUILD plan dependencies
5. /redesign/BUSINESS-COMPLETENESS.md — to identify any Track A items this page contributes to
6. The post-polish screenshots at /redesign/screenshots/role-detail-redesign/role-detail-polish-final-{375,768,1440}.png
7. The current source code: src/app/admin/role-detail/** and any other files in the recipe's "Files to edit" list
...
Task: invoke the impeccable Skill with `audit role-detail`. Score 5 dimensions and surface all P0/P1/P2/P3 findings with file:line references.

Return format — the full audit text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## role-detail — audit`, with these required subsections:
- 5 dimension scores
...
2. Append it verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## role-detail — audit`.
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (`role-detail` already substituted):

```
You are critiquing the redesign of admin page role-detail for Phase 6. The page has been crafted + polished + adapted + hardened + audited by another agent. Your job is an objective UX critique — you have NO bias from doing the work.

Re-prime (read in full):
1. /redesign/briefs/role-detail-brief.md
2. PRODUCT.md
3. DESIGN.md (full)
4. The post-polish screenshots at /redesign/screenshots/role-detail-redesign/role-detail-polish-final-{375,768,1440}.png
5. The current source code: src/app/admin/role-detail/**

Task: invoke the impeccable Skill with `critique role-detail`. Return:
...
Return format — the full critique text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## role-detail — critique`.

Do NOT write to PER-PAGE-SCORES.md. Return the full critique text verbatim.
```

After the subagent returns:
1. Append verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## role-detail — critique`.
2. **Print to chat verbatim** — same reasoning as 12a.
3. If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder role-detail` or `/impeccable distill role-detail` (whichever fits the verdict's reasoning), then re-dispatch the critique subagent with the same prompt. Loop max 2 times. If after 2 loops the verdict is still REGRESSED/FAIL, append the verdict + reasoning to `/redesign/per-page-deferrals/role-detail-deferrals.md` with **Defer to: Phase 7** and proceed to 12c.

### 12c — Functional smoke test

Run through brief's Feature Preservation Manifest manually via Playwright + read-the-code:

- [ ] `RoleMetadataForm` keeps `role_id` / `display_label` / `description` / `sort_order` / `active` field names + hidden `active=on` shadow for system roles
- [ ] `PermissionRow` Switch still calls `toggleRolePermission(roleId, permissionId, permissionName)` with the signature intact
- [ ] Owner-role permission toggle gates through `ConfirmActionModal` regardless of risk level
- [ ] `risk_level=critical` permission toggle ALWAYS confirms; `high` confirms on grant only; `medium` and `low` are one-click
- [ ] Permission toggle failure rolls back the Switch state and surfaces a Cancelled Sonner with Retry
- [ ] Metadata save success revalidates the path; header reflects new `display_label` without a full reload
- [ ] Deactivate role → confirm → chip swaps Active → Inactive; button label flips to `Reactivate role` (one-click)
- [ ] Delete role button visible only when `!is_system && staff_count === 0`; modal Cancelled family fires; success redirects to `/admin/roles` with Sonner `Role "{display_label}" deleted.` (under FAKE: degrade gracefully — disabled button + inline note, or hidden)
- [ ] Sticky category headers `position: sticky` on `lg:`+; on mobile, sticky disabled (per §10 Q3)
- [ ] Heading hierarchy: H1 (role name) → H2 ("Permissions") + H3 ("Role details" / "Staff with this role" / "Role lifecycle") contiguous
...
- [ ] PER-PAGE-SCORES.md sections appended (`## role-detail — audit` + `## role-detail — critique`) and printed to chat verbatim from the subagent results (Step 12a + 12b).
- [ ] Deferral file written at `/redesign/per-page-deferrals/role-detail-deferrals.md` — even if empty, write `(no deferrals — Phase 6 closed cleanly for role-detail)`. The main agent and the Phase 7 gauntlet both read this; missing file = ambiguous closure.
...
- Dev server URL: `http://localhost:3021/admin/role-detail`
...
9. `DEV_SERVER_READY at http://localhost:3021`
...
15. `## role-detail — audit` and `## role-detail — critique` headings appended (printed to chat from the file)
