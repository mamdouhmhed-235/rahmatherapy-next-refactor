...
# Raw px outside @media (allowed: tailwind arbitrary like `mt-[2px]` for icon alignment is borderline — flag, don't fail). NOTE: Recharts `minHeight: 288` literal is REQUIRED per brief §5 + BASELINE-CRITIQUE P1; do not refactor it into a token.
grep -nE '\[[0-9]+px\]' src/app/admin/reports/page.tsx
