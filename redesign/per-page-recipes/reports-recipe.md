...
Use `run_in_background: true`. Poll `http://localhost:3020/admin/reports` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree).
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/reports-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use the chrome-devtools MCP to read the last 20 console messages and print them to chat — verify 0 NEW errors vs `/redesign/BASELINE-ISSUES.md` (warnings OK). **The 6 Recharts ResponsiveContainer warnings flagged at baseline MUST be at 0 after this redesign (BASELINE-CRITIQUE P1 carry-forward).**
- Use the chrome-devtools MCP to inspect network requests during filter apply + CSV download — verify same endpoints as `/redesign/RECON.md` baseline (GET `/admin/reports` + GET `/admin/reports/export?report=<key>&...`)
...
7. `DEV_SERVER_READY at http://localhost:3020`
