...
- Append `step-0: COMPLETE — skills verified` to `/redesign/per-page-progress/roles-progress.md` and cat it.
...
```
/ralph-loop "Read the brief at /redesign/briefs/roles-brief.md. Compare the current implementation to the brief's requirements (5 native sections + Recipe Context + Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
Use `run_in_background: true`. Poll `http://localhost:3001/admin/roles` until it returns HTTP 200 (or 308). Max wait: 60 seconds.
...
**Action:** Use the `playwright` MCP tool (NOT `chrome-devtools` — playwright handles redirects). Sign in first with the owner test account (roles is owner-only per brief §11; `test.admin@rahmatherapy.example.test` will hit `AdminAccessDenied`). Resolve owner credentials from `/redesign/test-credentials.md`.
...
**Evidence to surface:**
- 5+ screenshot file paths printed to chat (`ls redesign/screenshots/roles-redesign/`)
- Literal decision: `ITERATE_DECISION: no refine needed — <brief-justified reason>` OR `ITERATE_DECISION: ran /impeccable <axis> roles because <reason>`
...
grep -nE '\\d+px' src/app/admin/roles/page.tsx
...
grep -nE '(margin\|padding):\s*\d' src/app/admin/roles/page.tsx
...
- Print network requests during the create-role flow — verify POST to `createRole` server action
...
## Step 12 — `/impeccable audit roles` + `/impeccable critique roles` + functional smoke test

### 12a — Audit
Invoke Skill with `/impeccable audit roles`.
...
Append to `/redesign/PER-PAGE-SCORES.md` under heading `## roles — audit`:
- 5 dimension scores
- P0/P1/P2/P3 findings, each on its own line
- Backend status: `N-A` (roles has no BLOCKS-REDESIGN backend dependencies)
...
### 12b — Critique
Invoke Skill with `/impeccable critique roles`.

Append to `/redesign/PER-PAGE-SCORES.md` under heading `## roles — critique`:
...
If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder roles` OR `/impeccable distill roles` based on which fits the verdict's reasoning, then re-run `/impeccable critique roles`. Loop max 2 times.
...
2. Compare changed-files list against `/redesign/per-page-scope/roles-scope.md`. For any file changed that isn't in the scope file's "Files to edit" list: STOP and emit `SCOPE_VIOLATION: <file>`. Otherwise: `SCOPE_CLEAN: only scoped files changed`.
...
4. Emit the handoff message to chat with:
   - Dev server URL: `http://localhost:3001/admin/roles`
