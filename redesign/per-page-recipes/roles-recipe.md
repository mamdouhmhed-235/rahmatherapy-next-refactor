...
Use `run_in_background: true`. Poll `http://localhost:3022/admin/roles` until it returns HTTP 200 (or 308). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/roles-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during the create-role flow — verify POST to `createRole` server action
...
8. `DEV_SERVER_READY at http://localhost:3022`
...
14. `## roles — audit` and `## roles — critique` headings appended (printed to chat from the file)
