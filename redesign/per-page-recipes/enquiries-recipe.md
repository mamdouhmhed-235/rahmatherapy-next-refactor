...
Use `run_in_background: true`. Poll `http://localhost:3015/admin/enquiries` until it returns HTTP 200 (or 308). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/enquiries-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during form submit + mark-contacted + filter apply — verify POSTs to `createEnquiry` and `updateEnquiryStatus` with the documented form field names (RECON §6.4 preserved)
...
9. `DEV_SERVER_READY at http://localhost:3015`
...
15. `## enquiries — audit` and `## enquiries — critique` headings appended (printed to chat from the file)
