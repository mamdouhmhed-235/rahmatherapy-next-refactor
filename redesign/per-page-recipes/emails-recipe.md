...
Use `run_in_background: true`. Poll `http://localhost:3014/admin/emails` until it returns HTTP 200 (or 308). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/emails-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during resend click — verify the POST to `sendManualBookingReminder` with `booking_id` in the form body (preserved verbatim) + GET refresh of the Reminders list
...
9. `DEV_SERVER_READY at http://localhost:3014`
...
15. `## emails — audit` and `## emails — critique` headings appended (printed to chat from the file)
