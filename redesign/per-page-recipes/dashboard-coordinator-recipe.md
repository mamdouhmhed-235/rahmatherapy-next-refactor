...
| Dev server port for this worktree | `3010` (user's main tree owns `3000`) |
...
1. The brief at `/redesign/briefs/dashboard-coordinator-brief.md` — quote the relevant section verbatim.
...
1. Append to `/redesign/per-page-deferrals/dashboard-coordinator-deferrals.md` in this format:
...
>   - [path] — [reason]
>
> **Unrendered-file rule:** Coordinator variant does NOT render every shared dashboard file (`demand-trend-client.tsx` is Owner/Admin-only; certain panels and the Export Ghost are revenue-gated). If a file listed under "Files to edit" is not actually rendered for the Coordinator role at any viewport, write the entry as `[path] — no changes in this session (component not rendered for Coordinator; carry-forward fixes already landed by dashboard-owner-admin OR will land when that session runs)`. This keeps Step 13 SCOPE_CLEAN honest and prevents the agent from inventing edits to justify the listing.
>
...
```
/ralph-loop "Read the brief at /redesign/briefs/dashboard-coordinator-brief.md. Compare the current implementation to the brief's requirements (every section of the brief, including Recipe Context and Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
1. Invoke the impeccable Skill tool with `<axis> dashboard-coordinator` args. Use the Skill tool (not the slash-command shorthand) so the invocation appears as a transcript event the Haiku evaluator can see.
2. After it completes, take `chunk1-1440-after-<axis>.png` at 1440×900 and save to `/redesign/screenshots/dashboard-coordinator-redesign/`.
...
After all axes complete, take post-polish screenshots at all 3 viewports: `dashboard-coordinator-post-axes-{375,768,1440}.png` to `/redesign/screenshots/dashboard-coordinator-redesign/`.

**Evidence to surface:**
- All baseline + per-axis + post-polish screenshot file paths printed to chat (`ls redesign/screenshots/dashboard-coordinator-redesign/`)
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/dashboard-coordinator-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Final 3-viewport screenshots: `dashboard-coordinator-polish-final-{375,768,1440}.png` saved to `/redesign/screenshots/dashboard-coordinator-redesign/`
...
grep -nE '(margin|padding):[[:space:]]*[0-9]' src/app/admin/dashboard/page.tsx src/app/admin/dashboard/dashboard-cards.tsx
```
...
- Use the chrome-devtools MCP to inspect network requests during filter change — verify only GET navigation (no server action invocation)
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (the slug `dashboard-coordinator` is already substituted below — pass this prompt verbatim):

```
You are auditing the redesign of admin page dashboard-coordinator for Phase 6 of the Rahma Therapy admin redesign. The page has just been crafted, polished, adapted, and hardened by another agent. Your job is an objective code + design audit — you have NO bias from doing the work.

Re-prime (read these in order, in full):
1. /redesign/briefs/dashboard-coordinator-brief.md
2. PRODUCT.md
3. DESIGN.md (full, including ## Admin-Specific Patterns)
4. /redesign/IMPLEMENTATION-PLAN.md — find the dashboard-coordinator row to determine Backend status (N-A / FAKE / HANDLED) and any BUILD plan dependencies
5. /redesign/BUSINESS-COMPLETENESS.md — to identify any Track A items this page contributes to
6. The post-polish screenshots at /redesign/screenshots/dashboard-coordinator-redesign/dashboard-coordinator-polish-final-{375,768,1440}.png
7. The current source code: src/app/admin/dashboard-coordinator/** and any other files in the recipe's "Files to edit" list
...
Task: invoke the impeccable Skill with `audit dashboard-coordinator`. Score 5 dimensions and surface all P0/P1/P2/P3 findings with file:line references.

Return format — the full audit text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## dashboard-coordinator — audit`, with these required subsections:
- 5 dimension scores
...
2. Append it verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## dashboard-coordinator — audit`.
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (`dashboard-coordinator` already substituted):

```
You are critiquing the redesign of admin page dashboard-coordinator for Phase 6. The page has been crafted + polished + adapted + hardened + audited by another agent. Your job is an objective UX critique — you have NO bias from doing the work.

Re-prime (read in full):
1. /redesign/briefs/dashboard-coordinator-brief.md
2. PRODUCT.md
3. DESIGN.md (full)
4. The post-polish screenshots at /redesign/screenshots/dashboard-coordinator-redesign/dashboard-coordinator-polish-final-{375,768,1440}.png
5. The current source code: src/app/admin/dashboard-coordinator/**

Task: invoke the impeccable Skill with `critique dashboard-coordinator`. Return:
...
Return format — the full critique text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## dashboard-coordinator — critique`.

Do NOT write to PER-PAGE-SCORES.md. Return the full critique text verbatim.
```

After the subagent returns:
1. Append verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## dashboard-coordinator — critique`.
2. **Print to chat verbatim** — same reasoning as 12a.
3. If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder dashboard-coordinator` or `/impeccable distill dashboard-coordinator` (whichever fits the verdict's reasoning), then re-dispatch the critique subagent with the same prompt. Loop max 2 times. If after 2 loops the verdict is still REGRESSED/FAIL, append the verdict + reasoning to `/redesign/per-page-deferrals/dashboard-coordinator-deferrals.md` with **Defer to: Phase 7** and proceed to 12c.
...
- [ ] PER-PAGE-SCORES.md sections appended (`## dashboard-coordinator — audit` + `## dashboard-coordinator — critique`) and printed to chat verbatim from the subagent results (Step 12a + 12b).
- [ ] Deferral file written at `/redesign/per-page-deferrals/dashboard-coordinator-deferrals.md` — even if empty, write `(no deferrals — Phase 6 closed cleanly for dashboard-coordinator)`. The main agent and the Phase 7 gauntlet both read this; missing file = ambiguous closure.
...
- Dev server URL: `http://localhost:3010/admin/dashboard-coordinator`
...
7. `DEV_SERVER_READY at http://localhost:3010`
...
11. `EXPORT_LINK_PRESENT: false` (Coordinator-only — Step 8 evidence verifies the revenue-gated Export Ghost is absent in the DOM)
12. `TOKEN_DRIFT: 0` (or each drift explicitly addressed)
13. `CONSOLE_NEW_ERRORS: 0`
14. `## dashboard-coordinator — audit` and `## dashboard-coordinator — critique` headings appended (printed to chat from the file)
