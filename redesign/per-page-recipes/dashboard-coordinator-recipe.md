...
Use `run_in_background: true`. Poll `http://localhost:3010/admin/dashboard` (signed in as Coordinator) until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/dashboard-coordinator-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during filter change — verify only GET navigation (no server action invocation)
...
8. `DEV_SERVER_READY at http://localhost:3010`
...
12. `EXPORT_LINK_PRESENT: false` (Coordinator-only — Step 8 evidence verifies the revenue-gated Export Ghost is absent in the DOM)
13. `TOKEN_DRIFT: 0` (or each drift explicitly addressed)
14. `CONSOLE_NEW_ERRORS: 0`
15. `## dashboard-coordinator — audit` and `## dashboard-coordinator — critique` headings appended (printed to chat from the file)
