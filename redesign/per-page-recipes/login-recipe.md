...
| Dev server port for this worktree | `3016` (user's main tree owns `3000`) |
...
1. The brief at `/redesign/briefs/login-brief.md` — quote the relevant section verbatim.
...
1. Append to `/redesign/per-page-deferrals/login-deferrals.md` in this format:
...
```
/ralph-loop "Read the brief at /redesign/briefs/login-brief.md. Compare the current implementation to the brief's requirements (every section of the brief, including Recipe Context and Implementation Notes). Apply one focused improvement. Re-check against the brief. If all brief requirements are met, output <promise>PAGE-POLISH-COMPLETE</promise>." --max-iterations 8 --completion-promise "PAGE-POLISH-COMPLETE"
```
...
## Step 6 — Start dev server on port 3016 (in this worktree)

**Action:** node_modules is already junctioned. Start the dev server in the background:

**Pre-flight (do this BEFORE the cd):** verify the worktree directory exists. If `Test-Path` (PowerShell) or `[ -d ... ]` (bash) returns false on `C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-login-redesign`, emit `STUCK: 6 — worktree directory missing — re-run the worktree setup from LAUNCH-SHEET Section 1a` and STOP. Do not try to recreate the worktree from inside the agent.

```bash
cd "C:\Users\mamdo\Desktop\rahmatherapy - Copy\rahmatherapy-login-redesign"
pnpm next dev -p 3016
```

Use `run_in_background: true`. Poll `http://localhost:3016/admin/login` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree).

**Evidence to surface:**
- The HTTP status code from the readiness poll printed to chat
- Literal line `DEV_SERVER_READY at http://localhost:3016`
- Append `step-6: COMPLETE — dev server on 3016` and cat progress file
...
1. Invoke the impeccable Skill tool with `<axis> login` args. Use the Skill tool (not the slash-command shorthand) so the invocation appears as a transcript event the Haiku evaluator can see.
2. After it completes, take `chunk1-1440-after-<axis>.png` at 1440×900 and save to `/redesign/screenshots/login-redesign/`.
...
After all axes complete, take post-polish screenshots at all 3 viewports: `login-post-axes-{375,768,1440}.png` to `/redesign/screenshots/login-redesign/`.

**Evidence to surface:**
- All baseline + per-axis + post-polish screenshot file paths printed to chat (`ls redesign/screenshots/login-redesign/`)
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/login-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Final 3-viewport screenshots: `login-polish-final-{375,768,1440}.png` saved to `/redesign/screenshots/login-redesign/`
...
grep -nE '[0-9]+px' src/app/admin/login/LoginForm.tsx src/app/admin/login/page.tsx | grep -v '@media'
...
grep -nE '(margin|padding):[[:space:]]*[0-9]' src/app/admin/login/LoginForm.tsx src/app/admin/login/page.tsx
```
...
- Use the chrome-devtools MCP to inspect network requests during sign-in flow — verify same endpoints as `/redesign/RECON.md` baseline (POST to `signInAdmin` server action, then GET `/admin/dashboard`)
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (the slug `login` is already substituted below — pass this prompt verbatim):

```
You are auditing the redesign of admin page login for Phase 6 of the Rahma Therapy admin redesign. The page has just been crafted, polished, adapted, and hardened by another agent. Your job is an objective code + design audit — you have NO bias from doing the work.

Re-prime (read these in order, in full):
1. /redesign/briefs/login-brief.md
2. PRODUCT.md
3. DESIGN.md (full, including ## Admin-Specific Patterns)
4. /redesign/IMPLEMENTATION-PLAN.md — find the login row to determine Backend status (N-A / FAKE / HANDLED) and any BUILD plan dependencies
5. /redesign/BUSINESS-COMPLETENESS.md — to identify any Track A items this page contributes to
6. The post-polish screenshots at /redesign/screenshots/login-redesign/login-polish-final-{375,768,1440}.png
7. The current source code: src/app/admin/login/** and any other files in the recipe's "Files to edit" list
...
Task: invoke the impeccable Skill with `audit login`. Score 5 dimensions and surface all P0/P1/P2/P3 findings with file:line references.

Return format — the full audit text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## login — audit`, with these required subsections:
- 5 dimension scores
...
2. Append it verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## login — audit`.
...
**Action:** Use the Agent tool with `subagent_type=general-purpose`. Subagent prompt (`login` already substituted):

```
You are critiquing the redesign of admin page login for Phase 6. The page has been crafted + polished + adapted + hardened + audited by another agent. Your job is an objective UX critique — you have NO bias from doing the work.

Re-prime (read in full):
1. /redesign/briefs/login-brief.md
2. PRODUCT.md
3. DESIGN.md (full)
4. The post-polish screenshots at /redesign/screenshots/login-redesign/login-polish-final-{375,768,1440}.png
5. The current source code: src/app/admin/login/**

Task: invoke the impeccable Skill with `critique login`. Return:
...
Return format — the full critique text, formatted to be appendable to PER-PAGE-SCORES.md under heading `## login — critique`.

Do NOT write to PER-PAGE-SCORES.md. Return the full critique text verbatim.
```

After the subagent returns:
1. Append verbatim to `/redesign/PER-PAGE-SCORES.md` under heading `## login — critique`.
2. **Print to chat verbatim** — same reasoning as 12a.
3. If AI-slop verdict is REGRESSED or FAIL: re-run `/impeccable bolder login` or `/impeccable distill login` (whichever fits the verdict's reasoning), then re-dispatch the critique subagent with the same prompt. Loop max 2 times. If after 2 loops the verdict is still REGRESSED/FAIL, append the verdict + reasoning to `/redesign/per-page-deferrals/login-deferrals.md` with **Defer to: Phase 7** and proceed to 12c.
...
- [ ] PER-PAGE-SCORES.md sections appended (`## login — audit` + `## login — critique`) and printed to chat verbatim from the subagent results (Step 12a + 12b).
- [ ] Deferral file written at `/redesign/per-page-deferrals/login-deferrals.md` — even if empty, write `(no deferrals — Phase 6 closed cleanly for login)`. The main agent and the Phase 7 gauntlet both read this; missing file = ambiguous closure.
...
- Dev server URL: `http://localhost:3016/admin/login`
...
7. `DEV_SERVER_READY at http://localhost:3016`
