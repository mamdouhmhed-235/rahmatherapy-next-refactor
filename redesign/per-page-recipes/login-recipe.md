...
Use `run_in_background: true`. Poll `http://localhost:3016/admin/login` until it returns HTTP 200 (or 308 — that's a trailing-slash redirect which Playwright handles). Max wait: 60 seconds.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/login-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use `chrome-devtools__list_network_requests` during sign-in flow — verify same endpoints as `/redesign/RECON.md` baseline (POST to `signInAdmin` server action, then GET `/admin/dashboard`)
...
8. `DEV_SERVER_READY at http://localhost:3016`
...
14. `## login — audit` and `## login — critique` headings appended (printed to chat from the file)
