...
Use `run_in_background: true`. Poll `http://localhost:3026/admin/staff` (then click into a staff member to reach `/admin/staff/<staffId>/availability`) until it returns HTTP 200 (or 308). Max wait: 120 seconds (cold compile of admin routes in Next.js 15 can exceed 60s — be patient on a fresh worktree). Resolve `<staffId>` by reading seed migrations or signing in as Owner and clicking through the staff directory.
...
**Re-audit, list remaining issues, fix again.** Loop maximum 2 iterations. If iteration 1 finds zero issues (the page already looks clean post-axes), emit `POLISH_ISSUES_ITER_1: none` AND `POLISH_ISSUES_ITER_2: none — clean (skipped, iteration 1 already clean)` and proceed directly to Step 8. If after 2 iterations there are still issues, append them to `/redesign/per-page-deferrals/staff-availability-deferrals.md` with **Defer to: Phase 7** and proceed.
...
- Use the chrome-devtools MCP to inspect network requests during the Panel A save + mode-switch flow — verify POSTs to the existing `StaffAvailabilityRulesForm` action + mode-update action
...
7. `DEV_SERVER_READY at http://localhost:3026`
