...
import { sendBookingReminderEmail } from "../notifications";

/**
 * C-08 Phase B. `sendBookingReminderEmail` now resolves "booking_reminder"
...
describe("sendBookingReminderEmail", () => {
  it("sends the reminder email to the client with the default copy", async () => {
    const stub = stubClient({ booking: baseBooking() });

    await sendBookingReminderEmail("booking-1", stub.client);

    expect(sendEmail).toHaveBeenCalledTimes(1);
    const call = vi.mocked(sendEmail).mock.calls[0][0];
    expect(call.to).toBe(CUSTOMER_EMAIL);
    expect(call.subject).toBe("Rahma Therapy Test booking reminder");
    expect(call.html as string).toContain(
      "Hi Aisha Khan, this is a reminder for your upcoming Rahma Therapy Test appointment."
    );
  ...
  it("throws before sending when the booking has no email anywhere", async () => {
    ...
    await expect(sendBookingReminderEmail("booking-1", stub.client)).rejects.toThrow(/email/i);
    expect(sendEmail).not.toHaveBeenCalled();
  });

  it("applies the admin-configured intro override to both the HTML and plain-text legs", async () => {
    const overrideRows = [
      { field_key: "intro", value: "OVERRIDE — see you on {bookingDate}, {clientName}." },
    ];
    ...
    expect(html).toContain("OVERRIDE — see you on 2026-07-20, Aisha Khan.");
    expect(html).not.toContain("this is a reminder for your upcoming");
