...
//
// C-14 Phase C dropped the unique constraint on (staff_id, override_date), so an adjusted
...
describe("groupAndCapStaffOverridesByDate", () => {
  it("counts a date with three segments as ONE toward the total, not three", () => {
    const { dateTotal } = groupAndCapStaffOverridesByDate(ROWS, { rowTotal: ROWS.length });

    expect(dateTotal).toEqual({ kind: "exact", value: 3 });
  });

  it("caps by DATE, keeping every row of each kept date", () => {
    const { flattenedRows, dateTotal } = groupAndCapStaffOverridesByDate(ROWS, {
      dateCap: 2,
  ...
  it("never splits a date's segments across the cap boundary", () => {
    ...
    const { flattenedRows } = groupAndCapStaffOverridesByDate(ROWS, { dateCap: 1, rowTotal: ROWS.length });

    expect(flattenedRows.map((r) => r.id)).toEqual(["a", "b"]);
  });

  it("keeps every date when no dateCap is given", () => {
    const { flattenedRows, dateTotal } = groupAndCapStaffOverridesByDate(ROWS, { rowTotal: ROWS.length });
  ...
  it("reports a LOWER BOUND when the row ceiling truncated the fetch", () => {
    ...
    const { dateTotal } = groupAndCapStaffOverridesByDate(ROWS, { rowTotal: 9 });
  ...
  it("saturates independently of the date cap", () => {
    const { dateTotal } = groupAndCapStaffOverridesByDate(ROWS, { dateCap: 1, rowTotal: 9 });
