...
--
-- Addresses outside the free-travel areas become BOOKABLE. They arrive as
-- `pending` and an admin decides; admin discretion is the boundary, and there
-- is deliberately no hard outer limit in code (Owner decision, plan 8.1).
--
-- This removes the LAST database reference to business_settings.allowed_cities.
-- Verified immediately before writing this file: pg_proc returned exactly one
-- function referencing the column (this one) and pg_policies returned zero.
--
-- The body below is the LIVE definition, byte-verified against pg_proc.prosrc
-- (length 18038, md5 723789e4d27db734e6550830e967231c), with exactly one block
-- removed - 323 characters, one `raise exception` (19 -> 18) - and nothing else
-- altered:
--
--   if not exists (
--     select 1
...
-- required'` check immediately above it. A city is still mandatory; it just no
-- longer has to be one of ours. create_recurring_booking_series never checked
-- the city and is deliberately left alone.
--
-- v_settings is declared `public.business_settings%rowtype` and loaded with
-- `select *`, so it needs no edit here and follows the table through Step Z.
--
-- Rollback: re-apply the prior body verbatim from
-- supabase/migrations/20260727120000_c06_client_crud_hardening.sql as a NEW
-- migration. Never edit that applied file in place.
