-- G-05-06 / fix-list B5 -- a COMPLETED booking still blocked its time slot.
--
-- THE DEFECT
--   create_booking_request asks "is this therapist busy?" in two places, with
--   two different definitions of busy:
--
--     named-therapist path : b.status not in ('cancelled', 'no_show')
--     unassigned path      : b.status in ('pending', 'confirmed')
--
--   So a booking marked `completed` still consumed a named therapist's
--   capacity, while the same booking left the unassigned pool alone.
--
-- OWNER RULING (2026-08-19): a completed booking must NOT block its slot.
--   The unassigned path is therefore correct and the named path is wrong.
--
-- ⛔ WHY THIS IS CUSTOMER-VISIBLE, not a quiet under-count
--   src/lib/booking/availability.ts:596 fetches bookings with
--   .in("status", ["pending", "confirmed"]) -- the SAME rule as the unassigned
--   path. So the public calendar OFFERS the slot, the customer completes the
--   whole form, and only then does this function refuse it.
--
-- REPRODUCED LIVE 2026-08-19, three arms in one rolled-back transaction, one
-- ZZTEST- female therapist, all other staff parked:
--
--   A  prior booking `confirmed`, assigned  -> THREW P0001  ✅ control: the
--                                              busy-check does work
--   B  prior booking `completed`, assigned  -> THREW P0001  ⛔ THE DEFECT
--   C  prior booking `completed`, unassigned-> SUCCEEDED    ✅ other path is
--                                              already correct
--
--   Predicate probe on arm B's own fixture:
--       CURRENT  (not in cancelled,no_show) -> busy = true
--       PROPOSED (in pending,confirmed)     -> busy = false
--   so this one line flips exactly this case and nothing else.
--
-- ⛔ WHY THE POSITIVE LIST, NOT `not in (..., 'completed')`
--   Both behave identically today. `not in` is a list of statuses that do NOT
--   block, so a status added later starts blocking slots silently. `in` is a
--   list of statuses that DO block, so anything new defaults to harmless.
--   Same result now, safer later.
--
-- ⚠️  KNOWN CONSEQUENCE, accepted by the Owner: if a therapist marks a booking
--   `completed` BEFORE its end time, the slot reopens while they are still
--   there. That exposure already existed on the unassigned path; this makes
--   the two paths agree rather than introducing it.
--
-- HOW THIS FILE WAS BUILT
--   It is 20260819150206_fix_booking_buffer_midnight_wrap.sql with that single
--   line changed -- NOT a dynamic string-patch of the live body. That base was
--   proven equivalent to the live function first:
--       repo body, comments+whitespace stripped : a86b093c13e2c841aa77c63cdbc570e9 (14310)
--       live prosrc, same normalisation         : a86b093c13e2c841aa77c63cdbc570e9 (14310)
--       live prosrc raw md5 before this applies : c0d74f2454bff1778e692e54a1817e80 (20185)
--   Re-transmitting the file rather than patching means the repo file and the
--   live function become byte-identical afterwards, closing the §6 fidelity
--   gap recorded in README-MIGRATION-DRIFT.md for this function.
--
--   The trailing REVOKE/GRANT block is carried over deliberately: CREATE OR
--   REPLACE keeps existing grants, but restating them keeps this file a
--   complete, self-sufficient definition for a rebuild.
