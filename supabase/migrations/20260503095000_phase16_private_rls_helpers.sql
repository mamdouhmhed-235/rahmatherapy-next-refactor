...
create or replace function app_private.current_active_staff_id()
returns uuid
language sql
stable
security definer
...
create or replace function app_private.current_staff_has_permission(permission_name text)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where staff_profiles.auth_user_id = auth.uid()
      ...
      and (
        app_private.current_staff_has_permission('claim_assignments')
        or app_private.current_staff_has_permission('claim_bookings')
      )
  )
$$;

grant usage on schema app_private to authenticated;
grant execute on function app_private.current_active_staff_id() to authenticated;
grant execute on function app_private.current_staff_has_permission(text) to authenticated;
grant execute on function app_private.current_staff_can_claim_gender(public.staff_gender_type) to authenticated;

drop policy if exists "Active staff can read roles" on public.roles;
drop policy if exists "Active staff can read permissions" on public.permissions;
...
drop policy if exists "Service managers can read all services" on public.services;
drop policy if exists "Global availability managers can read availability_rules" on public.availability_rules;
drop policy if exists "Global availability managers can read blocked_dates" on public.blocked_dates;
drop policy if exists "Global availability managers can read availability_overrides" on public.availability_overrides;
drop policy if exists "Permitted staff can read staff_availability_rules" on public.staff_availability_rules;
drop policy if exists "Permitted staff can read staff_blocked_dates" on public.staff_blocked_dates;
drop policy if exists "Permitted staff can read staff_availability_overrides" on public.staff_availability_overrides;
drop policy if exists "Settings managers can read business_settings" on public.business_settings;
...
drop policy if exists "Permitted staff can read booking_participants" on public.booking_participants;
drop policy if exists "Permitted staff can read booking_items" on public.booking_items;
drop policy if exists "Permitted staff can read booking_assignments" on public.booking_assignments;
drop policy if exists "Audit managers can read audit_logs" on public.audit_logs;
...
to authenticated
using (
  app_private.current_staff_has_permission('manage_roles')
  or role_id = (
...
using (
  ...
  or app_private.current_staff_has_permission('manage_roles')
);
...
to authenticated
using (app_private.current_staff_has_permission('manage_services'));

create policy "Global availability managers can read availability_rules"
on public.availability_rules for select
to authenticated
using (app_private.current_staff_has_permission('manage_availability_global'));
...
to authenticated
using (
  app_private.current_staff_has_permission('manage_availability_global')
  or (
    staff_id = app_private.current_active_staff_id()
    and app_private.current_staff_has_permission('manage_availability_own')
  )
);
...
to authenticated
using (app_private.current_staff_has_permission('manage_settings'));

create policy "Client managers can read clients"
on public.clients for select
to authenticated
using (app_private.current_staff_has_permission('manage_clients'));
...
using (
  ...
  or (
    (
      app_private.current_staff_has_permission('manage_bookings_own')
      or app_private.current_staff_has_permission('view_own_bookings')
    )
    and exists (
      select 1
...
using (
  app_private.current_staff_has_permission('manage_bookings_all')
  or app_private.current_staff_has_permission('view_all_bookings')
  or assigned_staff_id = app_private.current_active_staff_id()
  or (
    status = 'unassigned'
...
to authenticated
using (app_private.current_staff_has_permission('manage_audit_logs'));
...
drop function public.current_staff_can_claim_gender(public.staff_gender_type);
drop function public.current_staff_has_permission(text);
drop function public.current_active_staff_id();
