-- A2 -- the travel buffer wrapped around midnight and blocked whole days.
--
-- ✅ APPLIED TO PRODUCTION 2026-08-19 as version 20260819150206
--    (name: fix_booking_buffer_midnight_wrap). Owner-approved.
--
-- THE DEFECT
--   The capacity fix added the buffer to both overlap predicates. The columns
--   are `time without time zone`, so the arithmetic WRAPS. Verified live:
--
--       time '00:10' - interval '30 min'  ->  23:40
--       time '23:50' + interval '30 min'  ->  00:20
--
--   Postgres OVERLAPS then NORMALISES the inverted pair, so the "padded window"
--   covered nearly the whole day instead of a sliver. Verified live:
--
--       select (time '10:00', time '11:00') overlaps
--              ((time '23:30' - interval '30 min')::time,
--               (time '23:59' + interval '30 min')::time);   -- TRUE
--
--   Consequence: ONE late booking made ordinary midday bookings that day fail
--   with "Not enough male/female therapists available".
--
--   Reachable today: normal hours are 08:00-20:00, but p_override_availability
--   skips the booking-window check, so a 23:45 booking could be created and
--   that date was then broken for public bookings. Danger band at the current
--   30-minute buffer: start_time < 00:30 or end_time > 23:30.
--
-- ⛔ WHY NOT THE OBVIOUS CLAMP
--   greatest(time '00:00', ...) / least(time '23:59:59', ...) DOES NOT WORK,
--   and was measured failing before this was written:
--
--       greatest(time '00:00',   (time '23:30' - interval '30 min')::time) -> 23:00
--       least   (time '23:59:59',(time '23:59' + interval '30 min')::time) -> 00:29
--
--   least() compares the ALREADY-WRAPPED value, so the pair stays inverted and
--   the bug survives. Confirmed: the clamped form still returned TRUE for a
--   10:00-11:00 request against a 23:30 booking.
--
-- THE FIX
--   Compare in minutes-since-midnight, which cannot wrap, mirroring
--   src/lib/booking/availability.ts:226 exactly:
--
--       start1 < end2 + buffer   AND   end1 > start2 - buffer
--
--   No clamping is needed: a negative lower bound is simply less than any real
--   start time, which is precisely how the read engine already behaves. This
--   also removes the last semantic difference between the read and write paths.
--
--   p_start_time and v_end_time cannot wrap: the existing "Booking must finish
--   on the same day after it starts" check above already rejects that case.
--
-- ⛔ NOTHING ELSE IN THE FUNCTION WAS TOUCHED. Verified after applying:
--   the auth.role() guard, the advisory lock key (date only), the
--   unassigned-reservation subtraction and its ba.status / b.status filters,
--   SECURITY DEFINER, search_path = public, app_private, and the ACL
--   (postgres + service_role only; anon and authenticated both false).
--
-- APPLIED AS AN MD5-GUARDED PATCH OF THE LIVE SOURCE, not by re-transmitting
-- this file (gotcha 126). A read-only dry run predicted the whole outcome
-- first; each predicate was asserted to occur exactly once before replacing:
--
--       pre-apply  prosrc : md5 7bea3df6fdaf25bc8825c824b6b03967, length 20105... (20078)
--       predicted           md5 c0d74f2454bff1778e692e54a1817e80, length 20185
--       post-apply prosrc : md5 c0d74f2454bff1778e692e54a1817e80, length 20185  ✅
--       overlaps predicates remaining: 0
--
-- ⚠️ As with 20260819134224, this file and the live function are LOGICALLY
--    identical and TEXTUALLY different -- this file carries fuller comments.
--    See README-MIGRATION-DRIFT.md §6.
--
-- ROLLBACK
--   Re-apply 20260819134224_fix_booking_future_check_dst.sql, which is this
--   file with the two predicates reverted to the OVERLAPS form. ⛔ Doing so
--   reintroduces the midnight wrap.
  ...
  if not p_override_availability then
    ...
    loop
      ...
      select exists (
        ...
        where ba.assigned_staff_id = v_staff.id
          ...
          and (
            -- A2 (2026-08-19): minutes-since-midnight, mirroring overlaps() in
            -- src/lib/booking/availability.ts:226. `time` arithmetic WRAPS
            -- (00:10 - 30min = 23:40) and OVERLAPS then normalises the inverted
            -- pair, so the padded window covered nearly the whole day.
            extract(epoch from p_start_time) / 60
              < extract(epoch from b.end_time) / 60 + coalesce(v_settings.buffer_time_mins, 0)
            and extract(epoch from v_end_time) / 60
              > extract(epoch from b.start_time) / 60 - coalesce(v_settings.buffer_time_mins, 0)
          )
      )
    ...
    select
     ...
     where ba.assigned_staff_id is null
       ...
       -- A2 (2026-08-19): same midnight-wrap fix as the busy check above.
       and extract(epoch from p_start_time) / 60
             < extract(epoch from b.end_time) / 60 + coalesce(v_settings.buffer_time_mins, 0)
       and extract(epoch from v_end_time) / 60
             > extract(epoch from b.start_time) / 60 - coalesce(v_settings.buffer_time_mins, 0);
