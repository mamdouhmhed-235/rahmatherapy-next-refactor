...
**The Brand-Accent Containment Rule.** Brand Orange is contained to six decoration roles: (1) Cormorant numerals ≥1.5rem on KPI tiles and page H1 titles; (2) chart accent fills; (3) illustration accents (the warm orange highlight inside empty-state SVGs); (4) Burnt Orange focus rings; (5) leading 6×6 dots before `.admin-eyebrow` uppercase labels (label text stays in Soft Slate for WCAG); (6) link-hover underline color (underline decoration only — text color stays compliant). Never as background-with-text — the Pending family (amber, h=75) and Attention family (warm orange, h=65) own that pattern in their own hues. Containment by *pattern* (large display type, leading dot, decoration line) keeps Brand Orange visually distinct from the Pending / Attention chips that share an adjacent hue family. The Brand-Orange page-header accent line (5rem × 2px under every H1) is a seventh role, governed by this rule as "decoration-only signature."
...
**The Cormorant Exception.** Cormorant Garamond appears on (a) marquee dashboard KPI numerals and (b) page H1 titles only — the brand's two editorial anchors. Setting body copy, card titles, form labels, section H2s, or any other surface in Cormorant collapses the marquee character into decorative noise. The H1 + numeral pair gives every page an editorial anchor without the serif spreading across the system. Page H1 colour: Chronicle (same as Urbanist H1 would have been). Numeral colour: Brand Orange (the literal logo "RAHMA" hue).
...
- **Don't** use decorative blobs, glassmorphism, or background blur as a stylistic default. **Modal / dialog / sheet / command-palette backdrops are the documented carve-out:** the existing recipe `bg-[oklch(12%_0.01_165)]/35 backdrop-blur-sm` (and the heavier `backdrop-blur-md` on the sticky dashboard filter strip) is accepted as the admin's overlay-separation convention. The blur is paired with a tinted-neutral overlay, never used alone as a decorative wash, and only on transient overlay surfaces. Anything outside that scope (card resting state, panel surface, navigation chrome) stays blur-free.
...
- **View Tabs** (`/admin/bookings` ships 10: Needs Attention, Today, Upcoming, Claimable, …) — horizontal pill row above the list. Active: Therapy Blue fill, Field White text, Urbanist 500 label step. Inactive: transparent, Practice Charcoal, Work Sans 500. Hover: Hover Mist fill. Active tab **must** carry `aria-current="page"` (currently colour-only — Phase 6 fix per BASELINE-CRITIQUE Sam #3). On narrow viewports, view tabs become a momentum horizontal-scroll strip; never a stacked list.
